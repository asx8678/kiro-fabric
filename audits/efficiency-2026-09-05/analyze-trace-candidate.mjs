...
const renderText = (report) => {
  ...
  for (const row of report.bridgeTable) {
    lines.push(`  ${row.ref.padEnd(28)} n=${String(row.count).padStart(3)} total=${formatUs(row.totalUs).padStart(8)} p95=${formatUs(row.p95Us).padStart(8)} max=${formatUs(row.maxUs).padStart(8)} args=${row.argsChars}chars result=${row.resultChars}chars errors=${row.errors}`);
  }
  lines.push("");
