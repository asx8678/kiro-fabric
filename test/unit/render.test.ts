...
describe("text renderer", () => {
  ...
  it("renders run headers, results, and failures", () => {
    ...
    const failure = {
      ...
      runId: "run_bad",
      status: "failed",
      error: { code: "TYPECHECK_FAILED", message: "bad program", diagnostics: [{ line: 2, column: 1, message: "bad" }] },
    } as RunEnvelope;
    expect(renderRunText({ body, envelope: failure }, { color: false })).toContain("✗ TYPECHECK_FAILED: bad program");
    expect(renderRunText({ body, envelope: failure }, { color: false })).toContain("error: bad");
  });

  it("renders check status and warning carets", () => {
    expect(renderCheckText({ ok: true, diagnostics: [{ code: 1, category: "warning", message: "style", line: 1, column: 1 }] }, "return 1;", { color: false }))
      .toContain("✓ program valid · 1 lines");
    expect(renderCheckText({ ok: true, diagnostics: [{ code: 1, category: "warning", message: "style", line: 1, column: 1 }] }, "return 1;", { color: false }))
      .toContain("warning TS1: style");
    expect(renderCheckText({
      ok: false,
      diagnostics: [{ code: 1, category: "error", message: "bad", line: 1, column: 1 }],
    }, "return 1;", { color: false })).toContain("✗ type check failed");
  });

  it("formats successful, failed, and repair call events", () => {
    expect(formatCallEvent({ role: "worker", inputChars: 1_200, outputChars: 340, elapsedMs: 1_200, exitCode: 0 }, { color: false }))
      .toBe("› worker · 1200 in · 340 out · 1.2s");
    expect(formatCallEvent({ role: "worker", inputChars: 1, outputChars: 2, elapsedMs: 10, exitCode: 1, repair: true }, { color: false }))
      .toBe("✗ worker · 1 in · 2 out · 10ms · repair");
    expect(formatCallEvent({ role: "worker", inputChars: 1, outputChars: 2, elapsedMs: 10, exitCode: 0, repair: true }, { color: false }))
      .toBe("↻ worker · 1 in · 2 out · 10ms · repair");
  });
  ...
  it("uses a gold accent for brand, rules, gutters, and retry markers", () => {
    ...
    expect(plain).toContain("▸ fabric-lite run_gold · Demo · TypeScript · 2 lines · 5ms · 2 AI calls · 2 retries");
    expect(formatCallEvent({ role: "worker", exitCode: 0 }, { color: true })).toContain("[38;5;220m");
    expect(formatRunStart({ runId: "run_gold", body }, { color: false }))
      .toBe("▸ fabric-lite run_gold Demo · running 2 lines…");
