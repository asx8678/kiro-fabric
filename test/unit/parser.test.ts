...
describe("framing", () => {
  it("uses the canonical worker policy with an explicit envelope boundary", () => {
    expect(
      framePrompt({ instruction: "do it", context: "data" }).startsWith(
        `${loadPrompt("worker-agent").trimEnd()}\n\nFABRIC_REQUEST_V1_BEGIN\n`,
      ),
    ).toBe(true);
  });
  ...
  it("rejects malformed and schema-invalid values", () => {
    ...
    expect(() =>
      parseFramed("FABRIC_RESULT_BEGIN\n{}\nFABRIC_RESULT_END", {
        type: "object",
        required: ["x"],
      }),
    ).toThrow(/schema/);
  ...
  it("normalizes Kiro markdown-rendered frame markers", () => {
    const s =
      '\u001b[m> FABRIC_RESULT_BEGIN\u001b[0m\n{"ok":true}\nFABRIC\u001b[23mRESULT_END\u001b[0m';
    expect(
      parseFramed(s, { type: "object", properties: { ok: { const: true } }, required: ["ok"] }),
    ).toEqual({ ok: true });
  });
