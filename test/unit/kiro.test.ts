...
import { KiroHeadlessRunner, runProcess, workerEnv } from "../../src/runners/kiro.js";
import { chmod, mkdir, mkdtemp, readFile, rm, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import path from "node:path";

describe("worker environment", () => {
  it("strips cloud credentials so the guest worker cannot read them", () => {
    const env = workerEnv({});
    expect(env.KIRO_API_KEY).toBeUndefined();
    expect(env.AWS_SECRET_ACCESS_KEY).toBeUndefined();
    expect(env.AWS_SESSION_TOKEN).toBeUndefined();
    // Non-secret essentials are preserved.
...
describe("bounded child process capture", () => {
  it("caps stdout and stderr independently", async () => {
    const result = await runProcess(
      process.execPath,
      ["-e", "process.stdout.write('o'.repeat(80));process.stderr.write('e'.repeat(80))"],
      { timeoutMs: 5000, maxChars: 100 },
    );
  ...
  it("kills a child when either stream exceeds its cap", async () => {
    ...
    ).rejects.toThrow(/stdout limit of 64/);
  });
  ...
  it("keeps the chat prompt in documented positional input when stdin is unsupported", async () => {
    ...
    try {
      ...
      const result = await new KiroHeadlessRunner(executable).run({
        instruction: "stdin evidence",
        context: "",
        role: "general",
        maxOutputChars: 1000,
        timeoutMs: 5000,
      ...
      const invocation = JSON.parse(await readFile(record, "utf8")) as {
        args: string[];
        input: string;
      };
  ...
  it("resolves a bare executable through PATH across platforms", async () => {
    ...
    try {
      ...
      await expect(new KiroHeadlessRunner("fake-kiro-cli").doctor()).resolves.toMatchObject({
        ok: true,
  ...
  it("reports useful model JSON parse errors", async () => {
    ...
    try {
      ...
      await chmod(executable, 0o755);
      await expect(new KiroHeadlessRunner(executable).listModels()).rejects.toThrow(
