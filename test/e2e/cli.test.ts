...
describe("built CLI", () => {
  it("checks and executes deterministic bodies", () => {
    const check = spawnSync(process.execPath, [cli, "check", "--format", "json"], {
      input: "return {ok:true};",
      encoding: "utf8",
    });
    expect(check.status).toBe(0);
    expect(JSON.parse(check.stdout).status).toBe("valid");
    const atomic = spawnSync(process.execPath, [cli, "run", "--format", "json"], {
      input: "return {atomic:true};",
      encoding: "utf8",
    });
    expect(atomic.status).toBe(0);
    expect(JSON.parse(atomic.stdout)).toMatchObject({
      status: "succeeded",
      value: { atomic: true },
    });
    const invalid = spawnSync(process.execPath, [cli, "run", "--format", "json"], {
      input: "const x: string = 1; return x;",
      encoding: "utf8",
    });
    expect(invalid.status).toBe(2);
    expect(JSON.parse(invalid.stdout).error.code).toBe("TYPECHECK_FAILED");
    const run = spawnSync(process.execPath, [cli, "exec", "--format", "json"], {
      input: "return {ok: fabric.util.unique([1,1,2])};",
      encoding: "utf8",
    });
    expect(run.status).toBe(0);
    expect(JSON.parse(run.stdout)).toMatchObject({ status: "succeeded", value: { ok: [1, 2] } });
    const positional = spawnSync(process.execPath, [cli, "run", "--format", "json"], {
      input: 'const file = await fabric.fs.read("README.md"); return { path: file.path };',
      encoding: "utf8",
    });
    ...
    const noisy = spawnSync(process.execPath, [cli, "exec", "--format", "json"], {
      input: 'console.log("guest diagnostic"); console.error("guest stderr"); return {ok:true};',
      encoding: "utf8",
    });
    expect(noisy.status).toBe(0);
    expect(JSON.parse(noisy.stdout)).toMatchObject({ status: "succeeded", value: { ok: true } });
    expect(noisy.stderr).toContain("guest diagnostic");
    expect(noisy.stderr).toContain("guest stderr");
  });
  it("returns structured runtime errors for non-JSON final values", () => {
    for (const [body, detail] of [
      ["return undefined;", "undefined"],
      ["return 1n;", "bigint"],
      ["const value: {self?: unknown} = {}; value.self = value; return value;", "cyclic"],
      ...
      expect(run.status, body).toBe(1);
      expect(JSON.parse(run.stdout), body).toMatchObject({
        status: "failed",
        error: { code: "RUNTIME_FAILED" },
      });
      expect(run.stdout, body).toContain(detail);
    }
  });

  it("forwards filtered Kiro auth env without leaking values", () => {
    const root = mkdtempSync(path.join(tmpdir(), "fabric-env-"));
    try {
      const runner = path.join(root, "fake-kiro");
      writeFileSync(
        runner,
      ...
      const run = spawnSync(
        ...
        {
          input:
            'return await fabric.ai.run({instruction:"auth", outputSchema:{type:"object",required:["authenticated"]}});',
          encoding: "utf8",
          env: { ...process.env, KIRO_API_KEY: "seeded-do-not-log" },
        },
      );
      expect(run.status).toBe(0);
      ...
      expect(run.stdout + run.stderr).not.toContain("seeded-do-not-log");
    } finally {
  ...
  it("update-policy migrates an existing config without touching other settings", () => {
    ...
    try {
        ...
        JSON.stringify(
          {
            ...
            git: { allowCommit: false },
            mutation: { enabled: false, require: "clean", maxDiffChars: 30000 },
            cache: { enabled: false, maxEntries: 200, ttlMs: 0 },
            permissions: {
              read: "allow",
              commit: "ask",
              execute: "ask",
            ...
            shell: { enabled: false, allowedCommands: [], timeoutMs: 30000, maxOutputChars: 20000 },
      ...
      expect(
        JSON.parse(readFileSync(path.join(root, ".fabric-lite/config.json"), "utf8")).filesystem
          .allowWrite,
      ).toEqual(["src/**"]);
      // Apply.
      const applied = spawnSync(
        process.execPath,
        [cli, "update-policy", "--cwd", root, "--allow-write", "workspace", "--format", "json"],
        { encoding: "utf8" },
      );
      ...
      const noMode = spawnSync(
        process.execPath,
        [cli, "update-policy", "--cwd", root, "--format", "json"],
        { encoding: "utf8" },
      );
