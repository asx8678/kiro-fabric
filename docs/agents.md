...
When you supply a task, Fabric arms prewalk and immediately submits the task to Main. Without a task, it captures the next user input. Select the executor in `/fabric settings` under **Prewalk**. `prewalk.activation` is `"disabled"` by default, `"always"` for session-start plus repeat activation, or `"gated"` for deterministic selective activation. The conservative gate requires mutation intent and strong breadth evidence; it does not call a model. Automatic decisions are persisted as bounded, non-model telemetry without task text. See [configuration](configuration.md#prewalk-executor) for the exact rule and compatibility behavior.
...
This legacy council helper is not shipped by Kiro Fabric. Use direct bounded `agents.run()` fan-out and synthesize the results in the caller.
...
`agents.maxTokensPerChild` limits cumulative token use for each child. Its default value, `0`, disables the limit. The wall-clock `timeoutMs` limits time, and `budgetUsd` limits cost. Kiro ACP does not report the usage needed to enforce either accounting ceiling, so Kiro runs and managed Kiro subagents require both values to remain `0`; install/runtime preflights reject positive values. This limit caps the context of one runaway child before the host session compacts. Fabric stops the child with the same `timed_out` status and a `token limit` error. Recursive RLM is not part of Kiro Fabric.
