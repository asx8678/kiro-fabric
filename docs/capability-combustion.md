...
A `kiro-fabric-capability` custom message records the fired namespaces in `details`. Its `content` holds the words that echo removal needs. Each custom message also spends one unit of branch hint budget.
