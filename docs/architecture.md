...
In the default QuickJS runtime, guest code runs without `process`, `require`, filesystem, network, or subprocess globals. Every effect crosses the host bridge, and schemas, approvals, audit records, timeouts, and cancellation apply there. Each execution gets a fresh QuickJS context. Strings the caller names in the `strings` tool parameter appear as `π.key`. Reading a missing key throws a clear, actionable error that lists the provided keys. Aggregate `strings` payload, per-value size, and key count are bounded by `executor.maxSourceBytes` (and a 64-key cap) so named strings cannot bypass the source limit. Host-bridge JSON in both directions is bounded by `executor.maxNestedResultChars` (default 2,000,000 characters). Overlapping `write`/`edit`/`commit` file paths are reserved as active effects and fail closed even when `effectPolicy` is advisory; other overlapping footprints stay advisory unless a revertible component opts into `strict`. Independent paths stay parallel.
