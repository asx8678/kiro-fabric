...
3. Call `schema.commit`. Fabric requires both write and execute approval. The commit checks all bindings, captures durable before images, records every same-directory staging path, and consumes the certificate exactly once by compare-and-swap. It then computes and fsyncs every write/edit into a staging file before publishing any change. Source SHA-256 preconditions are rechecked both after staging and immediately before each atomic rename; a late mismatch refuses the commit. Deletes remain journaled operations. A failed stage, operation, undeclared drift, cancelled commit, or failed nonempty postcondition restores every declared path. Fabric records a failed rollback as `quarantined` and a successful one as `rolled_back`.
...
Writes and edits are prepared in same-directory temporary files (with the destination mode for replacements and owner-only mode for new files) and installed with atomic rename. No staged write/edit becomes visible when preparation of a later operation fails. The journal still provides transaction-level rollback because filesystems do not offer one atomic rename spanning multiple paths. Cancellation is checked before certificate consumption, throughout preparation/application, and after acceptance checks; cancellation after consumption rolls back.
...
Fabric stores hypotheses, certificates, gate reports, and outcomes as durable mesh records and events under `schema/*` and topic `fabric.schema`. Successful outcomes serialize through a workspace generation compare-and-swap on top of a cross-process commit lock. Before creating a staging file, Fabric atomically writes owner-only before images plus every planned staging path to the mesh transaction journal. On initialization, it cleans an interrupted `prepared` stage and restores an interrupted `applying` transaction; both paths remove recorded staging files. It also retries this recovery when lock acquisition discovers that an owner crashed after controller startup. An unreadable or unrestorable journal stays available for operator quarantine. Fabric consumes certificates by compare-and-swap before the first file operation, so a crash can never make one reusable.
