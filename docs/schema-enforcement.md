...
Fabric stores hypotheses, certificates, gate reports, and outcomes as durable mesh records and events under `schema/*` and topic `fabric.schema`. Successful outcomes serialize through a workspace generation compare-and-swap on top of a cross-process commit lock. Lock ownership is a random nonce in a complete, fsynced owner record; the fixed lock appears atomically as a hard link to that record. Release checks both nonce and inode. Stale recovery first hard-links the observed owner to a nonce-specific recovery marker and rechecks inode identity before unlinking, so a recovery contender cannot delete a successor's lock. Malformed or partially written legacy locks are never guessed stale.

Before creating a staging file, Fabric durably writes owner-only before images plus every planned staging/before-file path to the transaction journal. Before publication it adds the source and result digests and fsyncs the applying journal. On initialization, it cleans an interrupted `prepared` stage and conditionally restores an interrupted `applying` transaction; both paths remove recorded transaction files. Recovery restores only a destination still matching Fabric's recorded staged result (or an absent destination paired with Fabric's owned before-file). It preserves and reports all other content as quarantine. Restored bytes, restored modes, destination files, and affected parent directories are fsynced before a terminal journal status is written and fsynced. It also retries recovery when lock acquisition discovers that an owner crashed after controller startup. An unreadable or unrestorable journal stays available for operator quarantine. Fabric consumes certificates by compare-and-swap before the first file operation, so a crash can never make one reusable.
