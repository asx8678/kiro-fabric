...
- Every successful decision issues a 30-second, single-use lease. Fabric binds it to the resolved action descriptor and validated prepared arguments; `fabric_exec` leases are additionally bound to the exact type-checked program and project, while direct native calls are project-bound. The registry consumes the lease atomically at the final provider boundary. If native `tool_call` middleware rewrites arguments, Fabric requires and consumes a newly bound lease before native execution. Expired, replayed, concurrently reused, or mismatched leases fail before provider invocation, and a mismatch burns the lease. An explicit **Allow for this session**, inherited Kiro grant, or `allow` policy remains broad in authority, but each covered call still receives and consumes its own bound lease.
- Nested-call audits record only lease source, timestamps, action/risk, random lease ID, and SHA-256 binding digests. They never retain approval copies of arguments, program source, or project paths. Normal render audits continue to follow their documented argument/result retention policy.
...
`agents.runner` selects the default harness: `"pi"`, `"claude"`, `"veda"`, or `"kiro"`. An omitted runner inherits `"pi"`. An explicit unknown runner is rejected without rewriting the file. Version 4 is the compatibility fence that records `"kiro"` as a supported executable runner. Older Fabric binaries reject `configVersion` 4 without mutating it and never reinterpret Kiro as Pi. There is no automatic down-migration. `agents.model` is the optional Pi `provider/id` override. `agents.claude.model` is the optional canonical Claude runtime key. `agents.claude.binary` defaults to `claude`. You can supply an absolute path or a wrapper. `KIRO_FABRIC_CLAUDE_BINARY` overrides it for the current process. `/fabric settings` enumerates Claude models from that binary in the background and stores the two runner defaults independently.

The `kiro` runner drives Kiro CLI `2.20.1` over ACP (`kiro-cli acp --agent-engine v3 --auth-method cli`). Kiro v3 rejects the older ACP spawn-time `--agent`, `--model`, and `--effort` controls. Fabric instead injects its exact custom agent through `session/new._meta.kiro.customAgents`, injects the Fabric MCP server through `session/new.mcpServers`, activates `kiro-fabric` with `session/set_mode`, and applies model, effort, and supervised execution through `session/set_config_option` before the first prompt. It requires a managed `kiro-fabric install kiro` profile, never uses `--trust-all-tools`, and reports usage as unavailable. Each child materializes a run-scoped v3 profile whose only model-visible tool is `@fabric/fabric_exec`; that same object is sent over ACP, so KAS does not depend on discovering the file. The profile disables workspace MCP inheritance and Powers, and its v3 permission rules are empty unless the install used the exact Fabric-only `--allow-tools` grant. CLI-owned authentication requires preserving the authenticated OS `HOME`. With Kiro CLI 2.20.1/KAS 0.54.3, live probes also observed KAS transcripts under that OS home's `.kiro` session store even when `KIRO_HOME` was overridden. Durable actors therefore persist the session ID plus explicit v3 engine provenance and use `kiro-runtime-v3` only as a profile/engine marker; deleting the corresponding Kiro transcript makes resume fail closed. For Git worktrees, install the managed profile at the original repository root: Fabric keeps that root as the authority, then binds the per-run profile to the canonical effective worktree cwd (the worktree root or a subdirectory inside it). Missing worktrees or sibling cwd values outside the selected worktree fail closed. The requested bare `tools` list (`read`, `write`, `edit`, `bash`, `grep`, `find`, `ls`) is enforced host-side; unknown, namespaced, or prototype identifiers are rejected. Kiro runs keep one ACP process and session resident for bounded next-turn steering and follow-ups; queued messages become additional `session/prompt` calls on that same session. Fabric-owned compaction and recursion remain rejected. Because Kiro ACP reports no trusted usage accounting, both `agents.budgetUsd` and `agents.maxTokensPerChild` must be `0` (disabled) whenever Kiro is selected, including managed `--subagents`; runtime creation and installation fail before launch/write, and `kiro-fabric doctor kiro` reports the incompatible setting. Resume uses ACP `session/load` with the saved `runnerSessionId` and never silently starts a replacement session. Cancelled or timed-out first activations do not persist a new session id. `KIRO_FABRIC_KIRO_BINARY` overrides the default `kiro-cli` path.

The `veda` runner drives the [Veda CLI](https://github.com/kennyfrc/veda) as the child harness. `agents.veda.binary` defaults to `veda`. An absolute path or wrapper works, and `KIRO_FABRIC_VEDA_BINARY` overrides it for the current process. `agents.veda.backend` selects which backend Veda wraps: `agy` (Antigravity CLI, the default), `codex`, `claude-code`, `droid`, `pi`, or another backend registered by the installed Veda build. Fabric passes this value through unchanged and never hardcodes AGY. `agents.veda.model` is an optional backend-specific model or Veda alias. When you omit it, Veda selects its own backend default. `agents.veda.persona` picks the global Veda persona: `navigator-plan`, `navigator-chat` (default), `reviewer`, `worker`, or a custom persona under `~/.config/veda/personas/<name>/AGENTS.md`. Per-run selection overrides it through `agents.run({ persona })`. You can also edit the Veda backend, persona, and model in the Fabric settings panel under Agents. Each child runs one headless `veda --json` prompt with an isolated `fabric-<run-id>` session, so parallel children never share Veda selection or conversation state. Veda sessions lack persistence, and steering is unsupported. Veda children are **not** recursively Fabric-equipped (`recursive: true` is rejected), and they cannot back persistent actors.

A JS runtime launches each Fabric worker module. Fabric reuses the current runtime when `process.execPath` names `node` or `bun`. For a Bun-compiled Pi binary, `process.execPath` names the `pi` executable. Fabric then uses `KIRO_FABRIC_NODE_BINARY` or the first `node` or `bun` on `PATH`. The resolved runtime launches the workers. `KIRO_FABRIC_NODE_BINARY` overrides this choice for the current process. The Node-process executor (`executor.runtime: "node-process"`) requires Node.js because it uses `--eval` and `--input-type=module`.
...
- `sessionExportDir`: override the export store root (default `~/.kiro-fabric/agent`, with `KIRO_FABRIC_AGENT_DIR` taking precedence).
...
- **Isolated store**: to keep usage files fully outside pi's store, set `agents.sessionExportDir` (or `KIRO_FABRIC_AGENT_DIR`) to `~/.kiro-fabric/agent`, then register a ccusage named store for a dedicated `fabric` agent section:

  ```json
  { "pi": { "stores": [ { "name": "fabric", "path": "~/.kiro-fabric/agent/sessions/.fabric" } ] } }
  ```
