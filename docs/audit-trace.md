...
The final `fabric_exec` result details form a bounded durable envelope that holds `success`, `trace`, the rich render `audits`, `phases`, and `error`. The privacy-projected trace stays the functional record for compaction, memory, and tool ownership. The audits persist verbatim with full arguments, results, and previews. Approval-lease metadata is the exception: `audits[].approval` contains only action/risk, source, timestamps, lease ID, and descriptor/argument/plan/project digests; it never duplicates raw arguments, source code, or project paths. Composite grants (notably the independent write and execute grants for `schema.commit`) are validated completely and consumed all-or-none in one synchronous critical section. Every member gets a separate `audits[].approval[]` record immediately after consumption, so a later execution guard failure cannot hide an authorization that was spent. A resumed transcript re-renders and expands the same way as the live one. Collapsed cards provide the visual boundary. The record itself keeps the full data. Final session JSONL details never contain logs, return values, type errors, media payloads, or in-memory media notes.
