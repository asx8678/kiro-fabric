...
class PiCodingAgent(BaseInstalledAgent):
    """Pier adapter for paired Pi core and local kiro-fabric DeepSWE trials."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self._fabric_package_path and not self._fabric_package_path.is_file():
            raise ValueError(
                f"kiro-fabric package not found: {self._fabric_package_path}"
            )
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        if self._fabric_package_path:
            await environment.upload_file(
                self._fabric_package_path, "/tmp/kiro-fabric.tgz"
            )
            await self.exec_as_root(
                environment,
                command="npm install -g --ignore-scripts /tmp/kiro-fabric.tgz",
            )
    ...
    async def run(
        ...
        extension_flags = ""
        if self._fabric_package_path:
            extension_flags = '-e "$(npm root -g)/kiro-fabric"'
        else:
