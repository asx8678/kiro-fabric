...
var FABRIC_EXEC_RESULT_FORMATS = ["auto", "yaml", "json", "text"];
var fabricExecDisplayObject = typebox_exports.Object({
...
var fabricExecDisplayString = typebox_exports.String({
...
var fabricExecInputSchema = typebox_exports.Object({
  ...
  resultFormat: typebox_exports.Optional(
    typebox_exports.Union(FABRIC_EXEC_RESULT_FORMATS.map((value) => typebox_exports.Literal(value)))
  ),
  tokenBudget: typebox_exports.Optional(
  ...
  agentBudget: typebox_exports.Optional(
    typebox_exports.Integer({
      minimum: 1,
      description: "Optional agent-call cap, bounded by host capabilities and Fabric configuration"
    })
  ),
  display: typebox_exports.Optional(typebox_exports.Union([fabricExecDisplayObject, fabricExecDisplayString]))
}, { additionalProperties: false });
...
var prepareFabricExecArguments = (input) => {
  ...
  if (typeof display === "string" || isRecord(display)) {
    const normalized = normalizeRunDisplay(
      display
    );
...
// src/benchmark-boundary.ts
var BOUNDARY_SYMBOL = Symbol.for("kiro-fabric.benchmark-boundary.v1");
var boundary = () => {
  const value = globalThis[BOUNDARY_SYMBOL];
...
var benchmarkTreatmentDocument = () => boundary()?.document;
...
var writeFileAtomic = (filePath, contents, options) => {
  ...
  let descriptor;
  try {
    descriptor = fs.openSync(temporary, "wx", options?.mode ?? 384);
...
// src/agents/runners.ts
var FABRIC_AGENT_RUNNERS = ["pi", "claude", "veda", "kiro"];
var unsupportedFabricAgentRunnerMessage = (value) => `Unsupported Fabric agent runner: ${String(value)}`;
var isFabricAgentRunner = (value) => typeof value === "string" && FABRIC_AGENT_RUNNERS.includes(value);
var parseFabricAgentRunner = (value, fallback) => {
  if (value === void 0) {
    if (fallback === void 0) {
...
var assertSupportedConfigRunners = (document) => {
...
var runnerValue = (value, fallback) => parseFabricAgentRunner(value, fallback);
var prewalkModeValue = (value, fallback) => value === "in-place" || value === "trajectory" ? value : fallback;
var prewalkActivationValue = (value, fallback) => value === "always" || value === "gated" || value === "disabled" ? value : fallback;
  ...
  return {
    ...
    executor: {
      ...
      maxSourceBytes: boundedInteger(
        executor.maxSourceBytes,
        DEFAULT_FABRIC_CONFIG.executor.maxSourceBytes,
        MIN_EXECUTOR_SOURCE_BYTES,
        MAX_EXECUTOR_SOURCE_BYTES
      ),
...
var resolveFabricConfig = (options, includeProject, persistMigrations = true) => {
...
var inspectFabricConfig = (options) => resolveFabricConfig(options, options.projectTrusted, false);
var loadFabricConfig = (options) => {
  ...
  if (config.compaction.engine === "fabric") {
    process.env.KIRO_FABRIC_COMPACTION_ENGINE = "fabric";
  } else {
...
// src/kiro/accounting-compatibility.ts
var kiroAccountingIncompatibility = (agents, forceKiroRunner = false) => {
  if (!forceKiroRunner && agents.runner !== "kiro") return void 0;
  const enabled = [
    ...agents.budgetUsd > 0 ? ["agents.budgetUsd"] : [],
    ...agents.maxTokensPerChild > 0 ? ["agents.maxTokensPerChild"] : []
  ];
  return enabled.length === 0 ? void 0 : `Kiro ACP does not report usage and cannot enforce ${enabled.join(" or ")} > 0; set the listed ceilings to 0 or use a Pi, Claude, or Veda runner`;
};
var assertKiroAccountingCompatible = (agents, forceKiroRunner = false) => {
