...
var resolveKiroHome = (explicit) => {
  const raw = explicit ?? process.env.KIRO_HOME;
  const candidate = typeof raw === "string" && raw.trim() !== "" ? isAbsolute(raw) ? raw : resolve(process.cwd(), raw) : join(homedir(), ".kiro");
...
var resolveKiroInstallRoots = (options = {}) => {
...
var KIRO_ACP_AUTH_METHOD = "cli";
var KIRO_BINARY_ENV = "KIRO_FABRIC_KIRO_BINARY";
var KIRO_VERSION_ENV = "KIRO_FABRIC_KIRO_VERSION";
...
var kiroProfilePrompt = (allowShell, internalChild, enableSubagents) => {
  const mutationGuidance = allowShell ? "k.write, k.edit, and k.bash, which return objects shaped like { ok, output, details }. For code reviews or changes, run the smallest relevant tests/build with k.bash (set a sufficient timeout) and separate verified evidence from code-reading inferences." : "k.write and k.edit, which return objects shaped like { ok, output, details }. k.bash is disabled in this profile: do not call it or attempt shell execution.";
  const awaitedCalls = allowShell ? "k.ls, k.read, k.readArtifact, k.grep, k.find, k.write, k.edit, and k.bash" : "k.ls, k.read, k.readArtifact, k.grep, k.find, k.write, and k.edit";
  const agentGuidance = internalChild ? " Subagents are unavailable inside ACP children; do not call agents.*." : enableSubagents ? ' For independent work, agents.run and agents.spawn launch isolated Kiro ACP children with the same trusted shell access. Fan out at most four narrowly scoped, non-overlapping tasks in one fabric_exec program (prefer Promise.all), require each relevant child to run focused tests/builds, then cross-check and deduplicate findings in Main. Omit model for inventory-aware task routing: when advertised, small tasks use claude-haiku-4.5 at low effort, coding/testing uses qwen3-coder-next at low effort, and complex analysis or ambiguous tasks use claude-opus-4.8 at medium effort; otherwise routing falls back to Kiro auto. Pass model: "auto" to always let Kiro pick both model and effort. Do not delegate the same broad review to every child.' : " Subagents are disabled in managed Kiro: do not call agents.*.";
  const agentAwaitGuidance = !internalChild && enableSubagents ? " All agents.* API calls also return promises, so await every agents.* call." : "";
  const mcpGuidance = internalChild ? " MCP federation is unavailable inside ACP children; do not call mcp.*." : " Configured external MCP servers are available on demand only through await mcp.servers() and await mcp.call({ server, tool, args }). mcp.call performs external I/O and must pass Fabric's network approval gate; configured stdio executables also require the trusted execute grant. Do not use dynamic mcp.<server>.<tool> paths or register servers from guest code.";
  const memoryGuidance = internalChild ? " Persistent memory is unavailable inside ACP children; do not call memory.*." : " When enabled, project-isolated persistent facts are available through memory.get, memory.set, memory.search, and memory.index.";
  const additionalAwaitGuidance = internalChild ? "" : " All memory.* and supported mcp.* calls also return promises; await every one.";
...
var generateKiroProfile = (options) => {
  ...
  const confineManagedGrant = !internal && (options.allowShell === true || options.enableSubagents === true || options.allowTools === true);
  const runtimeNode = options.nodePath ?? process.execPath;
  let confinedProjectRoot;
  let projectIdentity;
    ...
    mcpServers: {
      fabric: {
        ...
        env: {
          ...
          ...options.kiroBinaryPath ? { [KIRO_BINARY_ENV]: options.kiroBinaryPath } : {},
          ...options.kiroCliVersion ? { [KIRO_VERSION_ENV]: options.kiroCliVersion } : {},
          ...options.kiroSha256 ? { [KIRO_SHA256_ENV]: options.kiroSha256 } : {},
          ...confineManagedGrant ? {
          ...
          ...!internal && options.skillBundleSha256 ? { KIRO_FABRIC_SKILL_BUNDLE_SHA256: options.skillBundleSha256 } : {},
          ...options.allowShell ? { KIRO_FABRIC_ALLOW_SHELL: "1" } : {},
          ...!internal && options.enableSubagents ? { KIRO_FABRIC_ENABLE_SUBAGENTS: "1" } : {},
          ...!internal && options.allowTools ? { KIRO_FABRIC_ALLOW_TOOLS: "1" } : {},
          ...internal ? {
...
var kiroProfilePath = (root, layout = "project") => managedPaths(root, layout).profile;

// src/kiro/compatibility.ts
...
// src/kiro/compatibility-test-seam.ts
import { readFileSync } from "node:fs";
import { createHash } from "node:crypto";
var activeFixtures;
var isPrivateKiroLauncherFixture = (_path, sha256) => activeFixtures?.some((fixture) => fixture.sha256 === sha256) === true;

// src/kiro/compatibility.ts
var execFileAsync = promisify(execFile);
var MIN_NODE_MAJOR = 24;
var KIRO_BINARY_ENV2 = "KIRO_FABRIC_KIRO_BINARY";
var KIRO_VERSION_ENV2 = "KIRO_FABRIC_KIRO_VERSION";
var KIRO_SHA256_ENV2 = "KIRO_FABRIC_KIRO_SHA256";
var PROBE_TIMEOUT_MS = 1e4;
var VERSION_TOKEN = /\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?/g;
var KIRO_IDENTITY = /^kiro-cli[ \t]+(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?)$/;
var NODE_IDENTITY = /^v?(\d+)\.(\d+)\.(\d+)$/;
var executableCandidates = (requested, pathValue) => {
  if (isAbsolute2(requested) || requested.includes("/") || requested.includes("\\")) {
    return [resolve2(requested)];
  }
  const directories = (pathValue ?? "").split(delimiter).filter(Boolean);
  if (process.platform !== "win32") return directories.map((directory) => resolve2(directory, requested));
  const extensions = (process.env.PATHEXT ?? ".EXE;.CMD;.BAT;.COM").split(";").filter(Boolean);
  return directories.flatMap(
    (directory) => extensions.map((extension) => resolve2(directory, requested + extension.toLowerCase()))
  );
};
var resolveCanonicalExecutable = (requested, pathValue = process.env.PATH) => {
...
var compareStableVersions = (left, right) => {
...
var classifyKiroVersionOutput = (output, requestedPath = "kiro-cli", executablePath = null) => {
  const text = output.trim();
  const tokens = text.match(VERSION_TOKEN) ?? [];
  const base = { requestedPath, executablePath, output: text, ok: false };
  if (tokens.length === 0) {
...
var failedProbeState = (error) => {
  const failure = error;
  return failure.killed === true || failure.code === "ETIMEDOUT" || failure.signal === "SIGTERM" ? "timeout" : "exec-failure";
};
var hasNativeExecutableFormat = (path) => {
  const header = readFileSync2(path).subarray(0, 4);
  ...
  if (process.platform === "darwin") {
    ...
    return magic === 4277009102 || magic === 4277009103 || magic === 3472551422 || magic === 3489328638 || magic === 3405691582 || magic === 3405691583 || magic === 3199925962 || magic === 3216703178;
  }
  if (process.platform === "win32") return header[0] === 77 && header[1] === 90;
  return false;
};
var stageKiroExecutable = (requestedPath) => {
  const resolved = resolveCanonicalExecutable(requestedPath);
  if ("error" in resolved) return resolved;
  const source = attestExecutable(resolved.path);
  if (!hasNativeExecutableFormat(source.path) && !isPrivateKiroLauncherFixture(source.path, source.sha256)) {
    return { error: "unsupported-launcher" };
  }
  const root = mkdtempSync(join2(tmpdir(), "kiro-fabric-kiro-stage-"));
  const stagedPath = join2(root, process.platform === "win32" ? "kiro-cli.exe" : "kiro-cli");
  try {
    const attestation = copyAttestedExecutable(source, stagedPath, 320);
    if (process.platform !== "win32") chmodSync(root, 320);
    let disposed = false;
    return {
...
var probeStagedKiro = async (requestedPath) => {
...
var inspectKiroCompatibility = async (requestedPath = "kiro-cli") => {
...
var describeKiroCompatibilityFailure = (report) => {
  const observed = report.output || report.version || report.requestedPath;
  switch (report.state) {
    case "ok":
      return "supported";
    case "not-found":
      return `Kiro CLI executable not found: ${report.requestedPath}`;
    case "not-executable":
      return `Kiro CLI path is not an executable regular file: ${report.requestedPath}`;
    case "unsupported-launcher":
      return `unsupported Kiro launcher artifact: ${report.requestedPath}; require a native ELF/Mach-O/PE executable or a complete attested launcher dependency closure`;
    case "timeout":
      return `Kiro CLI version probe timed out: ${report.requestedPath}`;
    case "exec-failure":
      return `Kiro CLI version probe failed: ${report.requestedPath}`;
    case "unparsable":
      return `unparsable kiro-cli version output ${JSON.stringify(observed)}`;
    case "ambiguous":
      return `ambiguous kiro-cli version output ${JSON.stringify(observed)}`;
    case "wrong-product":
      return `wrong product identity in kiro-cli version output ${JSON.stringify(observed)}`;
    case "prerelease":
      return `unsupported kiro-cli prerelease ${JSON.stringify(report.version)}; expected ${KIRO_CLI_VERSION}`;
    case "older":
      return `unsupported kiro-cli version ${JSON.stringify(report.version)}; expected ${KIRO_CLI_VERSION}`;
    case "newer":
      return `uncertified newer kiro-cli version ${JSON.stringify(report.version)}; expected ${KIRO_CLI_VERSION}`;
  }
};
var assertSupportedKiro = async (requestedPath = "kiro-cli") => {
  const probed = await probeStagedKiro(requestedPath);
...
var assertSupportedKiroUnchanged = (identity) => {
  assertExecutableAttestation(identity);
};
var classifyNodeVersion = (version, requestedPath, executablePath) => {
...
var inspectNodeCompatibility = async (requestedPath = process.execPath) => {
  const resolved = resolveCanonicalExecutable(requestedPath);
  if ("error" in resolved) {
    return { state: resolved.error, requestedPath, executablePath: null, version: null, ok: false };
  }
  if (resolved.path === realpathSync2(process.execPath)) {
...
var assertSupportedNode = async (requestedPath = process.execPath) => {
...
var sameExecutableIdentity = (requested, canonical) => {
...
function FromTypeObject(value) {
  ...
  for (const key of Object.keys(descriptors)) {
    ...
    if (guard_exports.HasPropertyKey(descriptor, "value")) {
      Object.defineProperty(result, key, { ...descriptor, value: FromValue2(descriptor.value) });
    }
  }
  return result;
}
function FromPlainObject(value) {
  ...
  for (const key of guard_exports.Keys(value)) {
    if (guard_exports.IsUnsafePropertyKey(key))
      continue;
    result[key] = FromValue2(value[key]);
  }
...
function FromArray2(value) {
  return value.map((element) => FromValue2(element));
}
...
function FromMap(value) {
  return new Map(FromValue2([...value.entries()]));
}
function FromSet(value) {
  return new Set(FromValue2([...value.values()]));
}
function FromValue2(value) {
  return globals_exports.IsTypeArray(value) ? FromTypedArray(value) : globals_exports.IsRegExp(value) ? FromRegExp2(value) : globals_exports.IsMap(value) ? FromMap(value) : globals_exports.IsSet(value) ? FromSet(value) : guard_exports.IsArray(value) ? FromArray2(value) : guard_exports.IsObject(value) ? FromObject2(value) : value;
}
...
function CapitalizeAction(type, options) {
  const result = CanInstantiate([type]) ? memory_exports.Update(FromType3(CapitalizeMapping, type), {}, options) : CapitalizeDeferred(type, options);
  return result;
}
function LowercaseAction(type, options) {
  const result = CanInstantiate([type]) ? memory_exports.Update(FromType3(LowercaseMapping, type), {}, options) : LowercaseDeferred(type, options);
  return result;
}
function UncapitalizeAction(type, options) {
  const result = CanInstantiate([type]) ? memory_exports.Update(FromType3(UncapitalizeMapping, type), {}, options) : UncapitalizeDeferred(type, options);
  return result;
}
function UppercaseAction(type, options) {
  const result = CanInstantiate([type]) ? memory_exports.Update(FromType3(UppercaseMapping, type), {}, options) : UppercaseDeferred(type, options);
  return result;
}
...
function FromIntersect(types) {
  return types.reduce((result, left) => {
    return CollapseIntersectProperties(result, FromType4(left));
  }, {});
}
...
function ReduceVariants(types, result) {
  return guard_exports.ShiftLeft(types, (left, right) => ReduceVariants(right, CollapseUnionProperties(result, FromType4(left))), () => result);
}
...
function FromType6(properties, type) {
  return IsArray2(type) ? _Array_(FromType6(properties, type.items)) : IsConstructor2(type) ? Constructor(FromTypes2(properties, type.parameters), FromType6(properties, type.instanceType)) : IsFunction2(type) ? _Function_(FromTypes2(properties, type.parameters), FromType6(properties, type.returnType)) : IsTuple(type) ? Tuple(FromTypes2(properties, type.items)) : IsUnion(type) ? Union(FromTypes2(properties, type.anyOf)) : IsIntersect(type) ? Intersect(FromTypes2(properties, type.allOf)) : IsThis(type) ? _Object_(properties) : type;
}
function ExpandThis(properties, type) {
  const result = FromType6(properties, type);
  return result;
}
...
function IndexAction(type, indexer, options) {
  const result = CanInstantiate([type, indexer]) ? memory_exports.Update(FromType7(NormalizeType(type), indexer), {}, options) : IndexDeferred(type, indexer, options);
  return result;
}
...
function FromTypes3(stack, context, types) {
  return guard_exports.ShiftLeft(types, (left, right) => FromType10(stack, context, left) ? true : FromTypes3(stack, context, right), () => false);
}
function FromType10(stack, context, type) {
  return IsRef(type) ? FromRef(stack, context, type.$ref) : IsArray2(type) ? FromType10(stack, context, type.items) : IsConstructor2(type) ? FromTypes3(stack, context, [...type.parameters, type.instanceType]) : IsFunction2(type) ? FromTypes3(stack, context, [...type.parameters, type.returnType]) : IsInterfaceDeferred(type) ? FromProperties(stack, context, type.parameters[1]) : IsIntersect(type) ? FromTypes3(stack, context, type.allOf) : IsObject2(type) ? FromProperties(stack, context, type.properties) : IsUnion(type) ? FromTypes3(stack, context, type.anyOf) : IsTuple(type) ? FromTypes3(stack, context, type.items) : IsRecord(type) ? FromType10(stack, context, RecordValue(type)) : false;
}
function CyclicCheck(stack, context, type) {
  const result = FromType10(stack, context, type);
  return result;
}
...
function FromTypes4(context, types, result) {
  return types.reduce((result2, left) => {
    return FromType11(context, left, result2);
  }, result);
}
function FromType11(context, type, result) {
  return IsRef(type) ? FromRef2(context, type.$ref, result) : IsArray2(type) ? FromType11(context, type.items, result) : IsConstructor2(type) ? FromTypes4(context, [...type.parameters, type.instanceType], result) : IsFunction2(type) ? FromTypes4(context, [...type.parameters, type.returnType], result) : IsInterfaceDeferred(type) ? FromProperties2(context, type.parameters[1], result) : IsIntersect(type) ? FromTypes4(context, type.allOf, result) : IsObject2(type) ? FromProperties2(context, type.properties, result) : IsUnion(type) ? FromTypes4(context, type.anyOf, result) : IsTuple(type) ? FromTypes4(context, type.items, result) : IsRecord(type) ? FromType11(context, RecordValue(type), result) : result;
}
function CyclicDependencies(context, key, type) {
  const result = FromType11(context, type, [key]);
  return result;
}
...
var KeyOf2 = (input) => If(If(If(Const("keyof", input), ([_0, input2]) => [[_0], input2]), ([_0, input2]) => [_0, input2], () => If([[], input], ([_0, input2]) => [_0, input2], () => [])), ([_0, input2]) => [KeyOfMapping(_0), input2]);
...
var With2 = (input) => If(If(If(Const("with", input), ([_0, input2]) => If(WithObject(input2), ([_1, input3]) => [[_0, _1], input3])), ([_0, input2]) => [_0, input2], () => If([[], input], ([_0, input2]) => [_0, input2], () => [])), ([_0, input2]) => [WithMapping(_0), input2]);
var Factor = (input) => If(If(KeyOf2(input), ([_0, input2]) => If(Base(input2), ([_1, input3]) => If(IndexArray(input3), ([_2, input4]) => If(Extends2(input4), ([_3, input5]) => If(With2(input5), ([_4, input6]) => [[_0, _1, _2, _3, _4], input6]))))), ([_0, input2]) => [FactorMapping(_0), input2]);
...
var Script = (input) => If(If(Module2(input), ([_0, input2]) => [_0, input2], () => If(GenericType(input), ([_0, input2]) => [_0, input2], () => If(Type(input), ([_0, input2]) => [_0, input2], () => []))), ([_0, input2]) => [ScriptMapping(_0), input2]);
...
function FromRecord2(inferred, _leftKey, leftValue, _rightKey, rightValue) {
  return ExtendsLeft(inferred, leftValue, rightValue);
}
function ExtendsRecord(inferred, leftPattern, leftValue, right) {
  return IsRecord(right) ? FromRecord2(inferred, RecordPatternToType(leftPattern), leftValue, RecordPatternToType(RecordPattern(right)), RecordValue(right)) : IsObject2(right) ? FromObject9(inferred, right.properties) : IsAny(right) ? ExtendsTrue(inferred) : IsUnknown(right) ? ExtendsTrue(inferred) : ExtendsFalse();
}
...
function FromProperties3(properties) {
  return guard_exports.Keys(properties).reduce((result, key) => {
    return { ...result, [key]: FromType17(properties[key]) };
  }, {});
}
function FromTypes5(types) {
  return types.reduce((result, left) => {
    return [...result, FromType17(left)];
  }, []);
}
function FromType17(type) {
  return IsRef(type) ? FromRef3(type.$ref) : IsArray2(type) ? _Array_(FromType17(type.items), ArrayOptions(type)) : IsConstructor2(type) ? Constructor(FromTypes5(type.parameters), FromType17(type.instanceType)) : IsFunction2(type) ? _Function_(FromTypes5(type.parameters), FromType17(type.returnType)) : IsIntersect(type) ? Intersect(FromTypes5(type.allOf)) : IsObject2(type) ? _Object_(FromProperties3(type.properties)) : IsRecord(type) ? Record(RecordKey(type), FromType17(RecordValue(type))) : IsUnion(type) ? Union(FromTypes5(type.anyOf)) : IsTuple(type) ? Tuple(FromTypes5(type.items)) : type;
}
...
export {
  ...
  kiroProfilePath,
  MIN_NODE_MAJOR,
  KIRO_BINARY_ENV2 as KIRO_BINARY_ENV,
  KIRO_VERSION_ENV2 as KIRO_VERSION_ENV,
  KIRO_SHA256_ENV2 as KIRO_SHA256_ENV,
