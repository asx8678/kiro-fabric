...
import { MAX_AGENT_STEER_LINE_BYTES } from "./steering-limits.js";
import { parseKiroChildTools } from "../kiro/run-scope.js";
import { parseAgentRunRecord, serializeAgentRunRecord } from "./run-record-codec.js";
import {
...
import {
  isAvailableUsage,
  unavailableUsage,
  type FabricBudgetSummary,
  type FabricSteeringMode,
  type FabricAgentLog,
  type AgentHandleInfo,
  type AgentRunRecord,
  type AgentRunRequest,
  type AgentRunResult,
  type AgentSteerEntry,
  type AgentSteerResult,
  type AgentTransportAdapter,
  type AgentTransportHandle,
  type AgentTransportLaunch,
} from "./types.js";
...
const readRecord = (filePath: string): AgentRunRecord | undefined => {
  try {
    return parseAgentRunRecord(JSON.parse(fs.readFileSync(filePath, "utf8")));
  } catch {
    return undefined;
  }
};
...
const writeRecord = (filePath: string, record: AgentRunRecord): void => {
  writeJsonAtomic(filePath, serializeAgentRunRecord(record), { space: 2 });
};
...
const failedRecord = (
  ...
  return {
    ...
    text: "",
    error,
    usage: unavailableUsage("worker-did-not-report"),
...
export class AgentManager {
  /** Authoritative insertion-ordered run storage. */
  readonly #runStore = new RunStore<ManagedAgent>();
  readonly #presenter = new RunPresenter<AgentRunRecord>(compactUiRecord);
  readonly #reconciler = new RunReconciler<AgentRunResult>();
  ...
  constructor(
    ...
    this.#runRoot =
      options.runRoot ?? process.env.KIRO_FABRIC_RUN_ROOT ?? fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-runs-"));
    ...
    this.#piBinary = options.piBinary ?? process.env.KIRO_FABRIC_PI_BINARY ?? "pi";
    this.#claudeBinary =
      options.claudeBinary ?? process.env.KIRO_FABRIC_CLAUDE_BINARY ?? config.claude.binary;
    this.#vedaBinary =
      options.vedaBinary ?? process.env.KIRO_FABRIC_VEDA_BINARY ?? config.veda.binary;
    this.#kiroBinary =
      options.kiroBinary ?? process.env.KIRO_FABRIC_KIRO_BINARY ?? "kiro-cli";
    ...
    this.#currentDepth = Math.max(0, Number(process.env.KIRO_FABRIC_DEPTH ?? "0") || 0);
    this.#fullCodeMode = options.fullCodeMode ?? true;
    this.#mainAgentId =
      options.mainAgentId ?? process.env.KIRO_FABRIC_MAIN_AGENT_ID;
    this.#meshRoot = options.meshRoot ?? process.env.KIRO_FABRIC_MESH_ROOT;
    this.#projectRoot =
      options.projectRoot ?? process.env.KIRO_FABRIC_PROJECT_ROOT ?? cwd;
    this.#hostId = options.hostId ?? process.env.KIRO_FABRIC_HOST_ID;
    this.#identityId = options.identityId ?? process.env.KIRO_FABRIC_IDENTITY_ID;
  ...
  subscribeUi(listener: () => void): () => void {
    return this.#presenter.subscribe(listener);
  }
  ...
  /** Kiro CLI binary used for non-billable model discovery probes. */
  ...
  async spawn(request: AgentRunRequest, signal?: AbortSignal): Promise<AgentHandleInfo> {
    ...
    if (runner === "kiro" && request.recursive) {
      throw new Error(
        "Kiro runner does not support recursive Fabric. Use a Pi runner for recursive: true, or omit recursive for Kiro ACP workers.",
      );
    }
    if (request.sessionSeed && runner !== "pi") {
      throw new Error("Trajectory handoff sessions are only supported by the Pi runner");
    }
    if (runner === "kiro" && this.config.maxTokensPerChild > 0) {
      throw new Error(
        "Kiro runner does not report token usage; set agents.maxTokensPerChild to 0 or use a Pi, Claude, or Veda runner",
      );
    }
    if (runner === "kiro" && this.#budget) {
      throw new Error(
        "Kiro runner does not report usage and cannot enforce an active Fabric cost budget",
      );
    }
    ...
    // One-shot Kiro workers do not get a steer channel: an unconditional steer
    // file makes every `agents.run` wait for the resident idle tail before
    // settling. Only resident launches retain follow-up capability.
    const kiroResidency = request.kiroResidency ?? "resident";
    const model =
      request.model ??
      (runner === "claude"
    ...
    const imagesFile = request.images && request.images.length > 0
      ...
      : undefined;
    const kiroContextFile = request.kiroContext
      ? path.join(runDirectory, "kiro-context.json")
      : undefined;
    if (kiroContextFile) {
      fs.writeFileSync(kiroContextFile, JSON.stringify(request.kiroContext, null, 2), {
        encoding: "utf8",
        mode: 0o600,
      });
    }
    ...
    if (request.worktree) {
      try {
        // Kiro keeps the original managed projectRoot and also receives the
        // generated worktree/effective cwd, then creates an isolated per-run
        // profile bound to that execution root.
        const lease = await this.#worktrees.create(id, selectedCwd, name, {
          preserveSourceSubdirectory: request.cwd !== undefined,
          writeMode: request.writeMode ?? "read",
        });
        agentCwd = lease.cwd;
        branch = lease.branch;
        worktree = lease.path;
        // PR2 rec #1: baseline the repo integrity immediately before the child
        // starts, so post-run contamination can be detected (rejectContamination).
        try {
          await this.#worktrees.markStarted(id);
        } catch (baselineError) {
          // A non-git baseline is already guarded in create(); propagate so we
          // never run an implementation agent without its integrity point.
          release();
    ...
    try {
      ...
      const workerArguments = [
        ...
        ...(runner === "kiro" && kiroResidency === "one-shot"
          ? []
          : ["--steer-file", steerFile]),
        ...(kiroContextFile ? ["--kiro-context-file", kiroContextFile] : []),
        ...(runner === "kiro" ? ["--kiro-residency", kiroResidency] : []),
  ...
  list(): Array<AgentRunRecord | AgentHandleInfo> {
    return [...this.#runStore.keys()].map((id) => this.status(id));
  }

  listForUi(): Array<AgentRunRecord | AgentHandleInfo> {
    return this.#presenter.cachedList(() => {
      const runs = this.#runStore.valuesArray();
      ...
      return runs
        ...
        .map((managed) => {
          ...
          return structuredClone(
            this.#presenter.project(this.#withTransportMetadata(record, managed)),
          );
        });
    });
  }
  ...
  runDirectory(id: string): string | undefined {
    return this.#runStore.get(id)?.runDirectory;
  }
  ...
  #requireSteerable(id: string): void {
    const runner = this.#requireRun(id).runner;
    if (runner === "veda") {
      throw new Error(
    ...
    // Kiro now steers a live resident ACP session; run-status staleness is handled in #appendSteer.
  }
  ...
  compact(id: string, instructions?: string): AgentSteerResult {
    const managed = this.#requireRun(id);
    if (managed.runner === "claude" || managed.runner === "veda" || managed.runner === "kiro") {
      throw new Error(
        "Fabric agent compaction is only supported for Pi-runner children; Claude Code, Veda, and Kiro sessions cannot be compacted through Fabric.",
      );
    }
  ...
  #appendSteer(id: string, entry: Omit<AgentSteerEntry, "id" | "ts">): AgentSteerResult {
    ...
    if (encodedBytes > MAX_AGENT_STEER_LINE_BYTES) {
      throw new Error(
        `Fabric agent steering command is too large (${encodedBytes} bytes; maximum ${MAX_AGENT_STEER_LINE_BYTES})`,
      );
    }
  ...
  async close(): Promise<void> {
    this.#closing = true;
    this.#presenter.clearListeners();
    if (this.#retentionTimer) clearInterval(this.#retentionTimer);
    this.#retentionTimer = undefined;
    await this.#retentionSweep?.catch(() => undefined);
    const running = this.#runStore.valuesArray().filter((managed) => !managed.settled);
  ...
  async #runRetentionSweep(now = Date.now()): Promise<void> {
    ...
    const expired = this.#runStore.valuesArray().filter((managed) => {
    ...
    for (const managed of expired) {
      ...
      if (!fs.existsSync(managed.runDirectory)) removable.push(managed.id);
    }
    this.#runStore.deleteMany(removable);
    if (expired.length > 0) {
      this.#pruneRetainedUiRecords();
      this.#invalidateUiList();
    }
  ...
  async #waitForTransportExit(managed: ManagedAgent): Promise<void> {
    ...
    await this.#reconciler.waitForTransportExit(
      () => managed.transport.isAlive(),
      pollIntervalMs,
      TRANSPORT_EXIT_GRACE_MS * 7,
    );
  }
  ...
  async #monitor(managed: ManagedAgent, timeoutMs: number): Promise<void> {
    ...
    while (!managed.settled) {
      ...
      if (record && terminalStatuses.has(record.status)) {
        ...
        // PR2 rec #1: an isolated writer must not have its tracked tree moved by
        // any external process while it ran. Reject a successful run whose repo
        // baseline drifted; the original worker diagnostics are preserved for
        // inspection rather than silently discarded.
        if (record.status === "completed" && managed.worktree) {
          try {
            await this.#worktrees.rejectContamination(managed.id);
          } catch (contamination) {
            const note =
              contamination instanceof Error
                ? contamination.message
                : "worktree contamination detected after run";
            const contaminated = failedRecord(
              managed,
              "failed",
              `result rejected: ${note}`,
            );
            writeRecord(managed.statusFile, contaminated);
            this.#settle(
              managed,
  ...
  #settle(managed: ManagedAgent, result: AgentRunResult): void {
    if (!this.#reconciler.settleOnce(managed.id, result, () => {
  ...
  #settleBudgetGap(managed: ManagedAgent, result: AgentRunResult): void {
    if (!isAvailableUsage(result.usage)) return;
  ...
  #childTools(request: AgentRunRequest, runner: FabricAgentRunner): string[] {
    ...
    if (runner === "kiro") {
      return parseKiroChildTools(tools);
    }
  ...
  async #resolveTransport(requested: FabricAgentTransport): Promise<AgentTransportAdapter> {
    return this.#executors.resolve(requested);
  }

  #pruneRetainedUiRecords(): void {
    const settled = this.#runStore.valuesArray().filter((managed) => managed.settled);
    const evicted = settled.slice(0, -MAX_RETAINED_RUN_HANDLES);
    this.#runStore.deleteMany(evicted.map((managed) => managed.id));
  ...
  #requireRun(id: string): ManagedAgent {
    return this.#runStore.require(id);
  }
