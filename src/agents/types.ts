...
export const AGENT_RUN_RECORD_FORMAT = 2 as const;

type AgentUsageAvailability = "available" | "unavailable";
export type AgentUnavailableReason =
  | "runner-does-not-report"
...
export interface AgentRunRequest {
  ...
  cwd?: string;
  /** Kiro ACP startup mode: resident runs keep the steer channel after the first turn. */
  kiroResidency?: "one-shot" | "resident";
  /** Omit the worker thinking flag so the runner (Kiro auto mode) picks effort. */
  suppressThinkingDefault?: boolean;
  /** Bounded context packet injected before the Kiro child's first turn. */
  kiroContext?: {
    objective?: string;
    facts?: string[];
  ...
  worktree?: boolean;
  /** Write access for an isolated worktree; exactly one writer per patch (rec #6). */
  writeMode?: import("./worktree-manager.js").WorktreeWriteMode;
...
export const isAvailableUsage = (usage: AgentUsage): boolean =>
...
export interface AgentRunRecord {
  format?: typeof AGENT_RUN_RECORD_FORMAT;
  id: string;
  name: string;
  task: string;
