...
const missingRuntimeError = (execPath: string): Error =>
  new Error(
    "Fabric requires a Node.js or Bun runtime to launch a JavaScript worker, but " +
      `process.execPath is ${execPath} (not node/bun) and KIRO_FABRIC_NODE_BINARY is unset. ` +
      "Install Node.js or Bun, or set KIRO_FABRIC_NODE_BINARY to the runtime binary.",
  );
