...
import { KIRO_AGENT_ENGINE } from "../kiro/profile.js";
...
interface ActorQueueItem {
  ...
  /** Completed model output retained while its delivery side effect retries. */
  outbox?: ActorMeshOutboxRecord;
}

interface ActorMeshOutboxRecord {
  message: FabricActorMessage;
  delivery: FabricActorDelivery;
  triggerTurn: boolean;
}

interface ActorMeshInboxRecord {
  format: 1;
  key: string;
  eventId: string;
  actorId: string;
  ...
  item: Omit<ActorQueueItem, "resolve" | "reject">;
  /** Completed model output persisted before any actor-to-Main side effect. */
  outbox?: ActorMeshOutboxRecord;
}
...
export class ActorManager {
  ...
  readonly #meshInboxPath: string | undefined;
  readonly #assertPersistentMutation: (() => void) | undefined;
  readonly #meshInbox = new Map<string, ActorMeshInboxRecord>();
  ...
  readonly #durableKiroResumeGates = new Map<string, Promise<void>>();
  ...
  constructor(
    ...
    readonly onDeliver: (request: FabricActorDeliveryRequest) => void | Promise<void>,
    options: {
      actorRoot?: string;
      persistent?: boolean;
      ...
      /** Exact host token/epoch fence checked immediately before durable writes. */
      assertPersistentMutation?: () => void;
    ...
    this.#actorRoot =
      options.actorRoot ?? fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-actors-"));
    ...
    this.#meshInboxPath = this.#persistent && this.meshConfig.enabled
      ? (options.meshCursorPath
          ? `${options.meshCursorPath}.inbox`
          : path.join(this.#actorRoot, "mesh-inbox.json"))
      : undefined;
    this.#assertPersistentMutation = options.assertPersistentMutation;
  ...
  async create(request: FabricActorRequest): Promise<FabricActorInfo> {
    ...
    const runner = parseFabricActorRunner(
      request.runner,
      isFabricActorRunner(this.agents.config.runner) ? this.agents.config.runner : undefined,
    );
  ...
  #acceptRelayedHostEvent(actor: ManagedActor, event: MeshEvent, cursor: number): void {
    ...
    this.#enqueue(actor, `host:${hostEvent}`, data.payload, {
      ...
      ...(images.length > 0 ? { images } : {}),
      ownershipChecked: true,
      durableMesh: { eventId: event.id, cursor },
    });
  }

  #serializableMeshItem(item: ActorQueueItem): Omit<ActorQueueItem, "resolve" | "reject"> {
    const { resolve: _resolve, reject: _reject, ...serializable } = item;
    return structuredClone(serializable);
  }

  #persistMeshInbox(records: Iterable<ActorMeshInboxRecord>): void {
    if (!this.#meshInboxPath) return;
    this.#assertPersistentMutation?.();
    fs.mkdirSync(path.dirname(this.#meshInboxPath), { recursive: true, mode: 0o700 });
    atomicWrite(this.#meshInboxPath, { format: 1, entries: [...records] });
  }

  #writeMeshInbox(): void {
    this.#persistMeshInbox(this.#meshInbox.values());
  }
  ...
  #recordMeshInbox(
    ...
    if (!this.#meshInboxPath) return;
    const key = `${eventId}:${actor.id}`;
    this.#meshInbox.set(key, {
      format: 1,
      key,
      eventId,
      actorId: actor.id,
      cursor,
      status: "pending",
      item: this.#serializableMeshItem(item),
    });
    try {
      // Journal durability precedes the in-memory queue push and mesh cursor.
      this.#writeMeshInbox();
    } catch (error) {
      this.#meshInbox.delete(key);
      throw error;
    }
  }

  #meshInboxOutbox(itemId: string): ActorMeshOutboxRecord | undefined {
    for (const record of this.#meshInbox.values()) {
      if (record.status === "pending" && record.item.id === itemId && record.outbox) {
        return structuredClone(record.outbox);
      }
    }
    return undefined;
  }

  #recordMeshInboxOutbox(item: ActorQueueItem, outbox: ActorMeshOutboxRecord): void {
    item.outbox = structuredClone(outbox);
    if (!this.#meshInboxPath) return;
    const itemId = item.id;
    const snapshot = new Map(this.#meshInbox);
    let changed = false;
    for (const [key, record] of snapshot) {
      if (record.status !== "pending" || record.item.id !== itemId) continue;
      snapshot.set(key, { ...record, outbox: structuredClone(outbox) });
      changed = true;
    }
    if (!changed) return;
    this.#persistMeshInbox(snapshot.values());
    this.#meshInbox.clear();
    for (const [key, record] of snapshot) this.#meshInbox.set(key, record);
  }

  #coalesceMeshInbox(
    actor: ManagedActor,
    existing: ActorQueueItem,
    next: ActorQueueItem,
    eventId: string,
    cursor: number,
  ): void {
    if (!this.#meshInboxPath) return;
    const serialized = this.#serializableMeshItem(next);
    const key = `${eventId}:${actor.id}`;
    const snapshot = new Map(this.#meshInbox);
    for (const [recordKey, record] of snapshot) {
      if (record.item.id === existing.id) {
        snapshot.set(recordKey, { ...record, item: structuredClone(serialized) });
      }
    }
    snapshot.set(key, {
      format: 1,
      ...
      cursor,
      status: "pending",
      item: structuredClone(serialized),
    });
    this.#persistMeshInbox(snapshot.values());
    for (const [recordKey, record] of snapshot) this.#meshInbox.set(recordKey, record);
  }

  #refillMeshInbox(actor: ManagedActor, outboxOnly = false): boolean {
    if (!this.#meshInboxPath || actor.status === "stopped") return false;
    const occupied = new Set(actor.queue.map((item) => item.id));
    if (actor.activeItemId) occupied.add(actor.activeItemId);
    const pending = [...this.#meshInbox.values()]
      .filter((record) =>
        record.actorId === actor.id &&
        record.status === "pending" &&
        (!outboxOnly || record.outbox !== undefined) &&
        !occupied.has(record.item.id),
      )
      .sort((left, right) =>
        left.item.activation.sequence - right.item.activation.sequence ||
        left.cursor - right.cursor ||
        left.key.localeCompare(right.key),
      );
    let added = false;
    for (const record of pending) {
      if (actor.queue.length >= this.meshConfig.actorQueueLimit) break;
      if (occupied.has(record.item.id)) continue;
      const restored = {
        ...structuredClone(record.item),
        ...(record.outbox ? { outbox: structuredClone(record.outbox) } : {}),
      };
      const insertionIndex = actor.queue.findIndex(
        (queued) => queued.activation.sequence > restored.activation.sequence,
      );
      if (insertionIndex === -1) actor.queue.push(restored);
      else actor.queue.splice(insertionIndex, 0, restored);
      occupied.add(record.item.id);
      actor.latestActivationSequence = Math.max(
        actor.latestActivationSequence,
        record.item.activation.sequence,
      );
      added = true;
    }
    if (added) {
      if (!actor.activeItemId) actor.status = "queued";
      actor.updatedAt = Date.now();
    }
    return added;
  }

  #actorMeshInboxItemIds(actorId: string): Set<string> {
    return new Set(
      [...this.#meshInbox.values()]
        .filter((record) => record.actorId === actorId)
        .map((record) => record.item.id),
    );
  }

  #completeMeshInboxItems(itemIds: ReadonlySet<string>): void {
    let changed = false;
    for (const [key, record] of this.#meshInbox) {
      if (!itemIds.has(record.item.id)) continue;
      if (record.cursor <= this.#meshOffset) this.#meshInbox.delete(key);
      else record.status = "completed";
      changed = true;
    }
    if (changed) this.#writeMeshInbox();
  }

  #completeMeshInboxItem(itemId: string): void {
    this.#completeMeshInboxItems(new Set([itemId]));
  }

  #ackMeshInbox(cursor: number): void {
    let changed = false;
    for (const [key, record] of this.#meshInbox) {
      if (record.cursor <= cursor && record.status === "completed") {
        this.#meshInbox.delete(key);
        changed = true;
      }
    }
  ...
  #restoreMeshInbox(): void {
    if (!this.#meshInboxPath) return;
    let parsed: unknown;
    try {
      parsed = JSON.parse(fs.readFileSync(this.#meshInboxPath, "utf8"));
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") return;
      throw new Error(`Invalid durable actor mesh inbox ${this.#meshInboxPath}: ${String(error)}`);
    }
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      throw new Error(`Invalid durable actor mesh inbox ${this.#meshInboxPath}`);
    }
    const entries = (parsed as { format?: unknown; entries?: unknown }).entries;
    if ((parsed as { format?: unknown }).format !== 1 || !Array.isArray(entries)) {
      throw new Error(`Unsupported durable actor mesh inbox ${this.#meshInboxPath}`);
    }
    let changed = false;
    for (const candidate of entries) {
      if (typeof candidate !== "object" || candidate === null || Array.isArray(candidate)) {
        throw new Error(`Malformed durable actor mesh inbox ${this.#meshInboxPath}`);
      }
      const record = candidate as Partial<ActorMeshInboxRecord>;
      const item = record.item as Partial<ActorQueueItem> | undefined;
      const activation = item?.activation as { sequence?: unknown } | undefined;
      const outbox = record.outbox as Partial<ActorMeshOutboxRecord> | undefined;
      const outboxMessage = outbox?.message as Partial<FabricActorMessage> | undefined;
      if (
        record.format !== 1 ||
        ...
        (outbox !== undefined && (
          !outboxMessage ||
          typeof outboxMessage.id !== "string" ||
          outboxMessage.actorId !== record.actorId ||
          outboxMessage.direction !== "out" ||
          outboxMessage.activationId !== item.id ||
          (outbox.delivery !== "mailbox" &&
            outbox.delivery !== "steer" &&
            outbox.delivery !== "followUp" &&
            outbox.delivery !== "nextTurn") ||
          typeof outbox.triggerTurn !== "boolean"
        )) ||
        record.key !== `${record.eventId}:${record.actorId}`
      ) {
        throw new Error(`Malformed durable actor mesh inbox ${this.#meshInboxPath}`);
      }
      if (record.status === "completed" && record.cursor <= this.#meshOffset) {
        changed = true;
        continue;
      }
      const actor = this.#actors.get(record.actorId);
      if (!actor || actor.status === "stopped") {
        changed = true;
        continue;
      }
      if (this.#meshInbox.has(record.key)) {
        throw new Error(`Duplicate durable actor mesh inbox key ${record.key}`);
      }
      this.#meshInbox.set(record.key, structuredClone(record as ActorMeshInboxRecord));
    }
    for (const actor of this.#actors.values()) this.#refillMeshInbox(actor);
    if (changed) this.#writeMeshInbox();
    queueMicrotask(() => {
      for (const actor of this.#actors.values()) {
  ...
  #writeMeshCursor(cursor: number): void {
    if (!this.#meshCursorPath) return;
    this.#assertPersistentMutation?.();
    atomicWrite(this.#meshCursorPath, { format: 1, cursor });
  }
  ...
  async stop(id: string): Promise<FabricActorInfo> {
    ...
    actor.updatedAt = Date.now();
    actor.abortController?.abort();
    const stoppedItems = actor.queue.splice(0);
    ...
    const stoppedItemIds = this.#actorMeshInboxItemIds(actor.id);
    if (actor.activeItemId) stoppedItemIds.delete(actor.activeItemId);
    for (const item of stoppedItems) stoppedItemIds.add(item.id);
    this.#completeMeshInboxItems(stoppedItemIds);
    await this.#publishPresence(actor);
    await this.mesh
      .publish({
        topic: "fabric.actor.lifecycle",
  ...
  haltAll(): { halted: number } {
    ...
    for (const actor of this.#actors.values()) {
      ...
      const haltedItemIds = this.#actorMeshInboxItemIds(actor.id);
      if (actor.activeItemId) haltedItemIds.delete(actor.activeItemId);
      if (!inFlight && actor.queue.length === 0 && haltedItemIds.size === 0) continue;
      ...
      const haltedItems = actor.queue.splice(0);
      for (const item of haltedItems) {
        item.reject?.(
          new Error(`Fabric actor ${actor.name} (${actor.id}) halted by user interrupt`),
        );
        haltedItemIds.add(item.id);
      }
      this.#completeMeshInboxItems(haltedItemIds);
      actor.updatedAt = Date.now();
  ...
  async close(): Promise<void> {
    ...
    options: ActorMessageBindingOptions & {
      ...
      durableMesh?: { eventId: string; cursor: number };
    } = {},
  ): ActorQueueItem {
    ...
    const durableMesh = options.durableMesh && this.#meshInboxPath
      ? options.durableMesh
      : undefined;
    if (durableMesh) {
      const key = `${durableMesh.eventId}:${actor.id}`;
      const existing = this.#meshInbox.get(key);
      if (existing) {
        if (existing.cursor !== durableMesh.cursor) {
          existing.cursor = durableMesh.cursor;
          this.#writeMeshInbox();
        }
        this.#refillMeshInbox(actor);
    ...
    if (options.coalesceKey) {
      const queuedExisting = actor.queue.find(
        (item) => item.coalesceKey === options.coalesceKey,
      );
      const backlogExisting = durableMesh && !queuedExisting
        ? [...this.#meshInbox.values()]
            .filter((record) =>
              record.actorId === actor.id &&
              record.status === "pending" &&
              record.item.coalesceKey === options.coalesceKey &&
              record.item.id !== actor.activeItemId &&
              !actor.queue.some((item) => item.id === record.item.id),
            )
            .sort((left, right) =>
              right.item.activation.sequence - left.item.activation.sequence,
            )[0]?.item
        : undefined;
      const existing = queuedExisting ?? backlogExisting;
      if (existing) {
        const next: ActorQueueItem = {
          ...existing,
          source,
          ...
          ...(options.images && options.images.length > 0
            ...
            : {}),
          createdAt,
          activation: this.#activation(existing.id, source, payload, sequence, createdAt),
          binding,
        };
        if (!options.images || options.images.length === 0) delete next.images;
        if (durableMesh) {
          this.#coalesceMeshInbox(
            actor,
            existing,
            next,
            durableMesh.eventId,
        ...
        if (queuedExisting) {
          if (!next.images) delete queuedExisting.images;
          Object.assign(queuedExisting, next);
        }
        this.#refillMeshInbox(actor);
        this.#ensureDrain(actor);
        return queuedExisting ?? next;
      }
    }
    if (!durableMesh && actor.queue.length >= this.meshConfig.actorQueueLimit) {
      throw new Error(
    ...
    if (durableMesh) {
      this.#recordMeshInbox(actor, item, durableMesh.eventId, durableMesh.cursor);
    }
    ...
    try {
      while (
        ...
        try {
          ...
          const replayOutbox = item.outbox ?? this.#meshInboxOutbox(item.id);
          if (replayOutbox) {
            runId = replayOutbox.message.runId;
            runCompleted = true;
            if (!actor.messages.some((candidate) => candidate.id === replayOutbox.message.id)) {
              this.#recordMessage(actor, replayOutbox.message);
            }
            deliveryPending = true;
            this.#recordMeshInboxOutbox(item, replayOutbox);
            await this.#deliverActorOutput(
              actor,
              item,
              replayOutbox.message,
              replayOutbox.delivery,
              replayOutbox.triggerTurn,
            );
            this.#settleActorOutput(actor, item, replayOutbox.message);
          ...
          const runnerSessionId =
            actor.runner === "kiro"
              ? this.#durableKiroRunnerSessionId(actor)
              : actor.runnerSessionId;
          const result = await this.#runDurableKiroActivation(
            actor,
            runnerSessionId,
            () => this.agents.run(
              this.#runRequest(
          ...
          if ((actor.runner === "claude" || actor.runner === "kiro") && result.runnerSessionId) {
            ...
            const shouldPersistRunnerSessionId =
              ...
              (result.status !== "stopped" &&
                ...
                (!actor.runnerSessionId || actor.runnerSessionId === nextRunnerSessionId));
            if (
              shouldPersistRunnerSessionId &&
              (actor.runnerSessionId !== nextRunnerSessionId || kiroProvenanceChanged)
            ) {
              actor.runnerSessionId = nextRunnerSessionId;
              await this.#saveActors();
            } else if (shouldPersistRunnerSessionId) {
              actor.runnerSessionId = nextRunnerSessionId;
            }
          }
          ...
          deliveryPending = true;
          this.#recordMeshInboxOutbox(item, {
            message,
            delivery: actor.delivery,
            triggerTurn: actor.triggerTurn,
          });
          ...
          this.#settleActorOutput(actor, item, message);
          deliveryPending = false;
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          if (
            deliveryPending &&
            this.#actorMeshInboxItemIds(actor.id).has(item.id)
          ) {
            // The model output is journaled on the inbox row. Leave that row
            // pending and stop this drain so a later poll/owner replays the
            // outbox without repeating model or tool side effects.
          ...
          if (!this.#canManage(actor.id) || this.#closing) {
            // Keep a durable mesh inbox item pending for the next fenced owner
            // or process restart. Direct-call promises are still rejected.
            requeued = true;
            item.reject?.(new Error(message));
            continue;
          }
          ...
          delete actor.activeItemId;
          actor.updatedAt = Date.now();
          if (!requeued) await this.#completeMeshInboxItem(item.id);
          if (
            !requeued &&
            !this.#closing &&
            this.#canManage(actor.id) &&
            (actor.status as FabricActorStatus) !== "stopped"
          ) {
            this.#refillMeshInbox(actor);
          }
          if ((actor.status as FabricActorStatus) !== "stopped") {
            actor.status = actor.queue.length > 0 ? "queued" : "idle";
          }
  ...
  async #runDurableKiroActivation<T>(
    ...
    if (!runnerSessionId || actor.runner !== "kiro" || actor.residency !== "durable") {
      return run();
    }
    const gateKey = `${actor.id}:${runnerSessionId}`;
    const inFlight = this.#durableKiroResumeGates.get(gateKey);
    if (inFlight) await inFlight;
    const gate = run();
    const completion = gate.then(() => undefined, () => undefined);
    this.#durableKiroResumeGates.set(gateKey, completion);
    try {
      return await gate;
    } finally {
      if (this.#durableKiroResumeGates.get(gateKey) === completion) {
        this.#durableKiroResumeGates.delete(gateKey);
      }
    }
  }
      ...
      actor.runner === "pi" && !fabricEnabled
        ...
        : actor.runner === "pi"
          ...
          : actor.runner === "claude"
            ...
            : "The Fabric host manages your mailbox, subscriptions, delivery, and lifecycle. This Kiro runner reaches Fabric only through the managed fabric_exec wrapper; coordinate through the messages the host delivers.";
  ...
  async #deliverActorOutput(
    ...
    if (
      ...
      let deliveryRequest: FabricActorDeliveryRequest | undefined;
      try {
        deliveryRequest = {
        ...
        // A non-cloneable payload cannot cross the delivery boundary.
      }
      if (deliveryRequest) await this.onDeliver(deliveryRequest);
    }
  }
  ...
  #settleActorOutput(
    ...
    actor.status = "stopped";
    const stoppedItems = actor.queue.splice(0);
    for (const queued of stoppedItems) {
      queued.reject?.(
        new Error(
    ...
    const stoppedItemIds = this.#actorMeshInboxItemIds(actor.id);
    stoppedItemIds.delete(item.id);
    for (const queued of stoppedItems) stoppedItemIds.add(queued.id);
    this.#completeMeshInboxItems(stoppedItemIds);
  }
  ...
  async #pollMesh(): Promise<void> {
    ...
    try {
      ...
      for (const event of tail.events) {
        ...
        else if (!event.topic.startsWith("fabric.control.")) {
          this.#dispatchMeshEvent(event, tail.nextOffset);
        }
      }
      // Every matching actor inbox record above is fsync/rename durable before
      // the cursor can acknowledge this batch. On failure the old offset stays
      // live and replay deduplicates by eventId+actorId.
      this.#writeMeshCursor(tail.nextOffset);
      this.#meshOffset = tail.nextOffset;
      this.#ackMeshInbox(tail.nextOffset);
      // A delivery-store failure leaves the completed model output journaled
      // on a pending inbox row and deliberately stops that actor's drain. A
      // later reconciliation must put the row back in memory even when no new
      // mesh event arrived, so the same live owner can retry delivery without
      // rerunning the model or waiting for a restart.
      for (const actor of this.#actors.values()) {
        if (!this.#canManage(actor.id) || actor.status === "stopped") continue;
        if (this.#refillMeshInbox(actor, true)) this.#ensureDrain(actor);
      }
    } finally {
      this.#polling = false;
    }
  ...
  #dispatchMeshEvent(event: MeshEvent, cursor: number): void {
    this.#refreshOwnership();
    for (const actor of this.#actors.values()) {
      if (!this.#canManage(actor.id) || actor.status === "stopped") continue;
      ...
      try {
        if (event.topic === RESIDENT_HOST_EVENT_TOPIC && addressed) {
          this.#acceptRelayedHostEvent(actor, event, cursor);
        } else {
        ...
        // Queue saturation or journal I/O means work is not durable: abort the
        // poll so the cursor remains behind this event and it can be retried.
        throw error;
      }
    }
  }
  ...
  #recordMessage(actor: ManagedActor, message: FabricActorMessage): void {
    ...
    if (serializedBytes(bounded) > maxPayloadBytes) {
      bounded = {
        ...
        ...(bounded.activationId ? { activationId: bounded.activationId } : {}),
  ...
  async #saveActors(removedIds: ReadonlySet<string> = new Set()): Promise<void> {
    ...
    await this.#withRegistryLock(() => {
      ...
      this.#assertPersistentMutation?.();
      atomicWrite(this.#registryPath, { format: 2, actors });
      this.#registryFingerprint = this.#currentRegistryFingerprint();
  ...
  #loadActors(onlyMissing = false): void {
    ...
    const fileFormat = (parsed as { format?: unknown }).format;
    if (fileFormat !== undefined && fileFormat !== 1 && fileFormat !== 2) return;
    ...
    for (const value of records) {
      ...
      const runner = record.runner === undefined
        ? "pi"
        : isFabricActorRunner(record.runner)
          ? record.runner
          : undefined;
      ...
      const actor: ManagedActor = {
        ...
        runner,
        ...((runner !== "kiro" || record.kiroAgentEngine === KIRO_AGENT_ENGINE) &&
        typeof record.runnerSessionId === "string" && record.runnerSessionId.trim()
          ? { runnerSessionId: record.runnerSessionId }
          : (() => {
              const retainedRunnerSessionId = this.#retainedRunnerSessionId(
                record.id,
                typeof record.lastRunId === "string" ? record.lastRunId : undefined,
                runner,
              );
              return retainedRunnerSessionId ? { runnerSessionId: retainedRunnerSessionId } : {};
            })()),
