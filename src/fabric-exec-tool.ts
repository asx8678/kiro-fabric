...
import type { FabricExecutionHost } from "./kiro/host.js";
import {
...
/**
 * Pi host: preserves the existing behavior byte-for-byte by delegating to the
 * real ExtensionContext, ApprovalController, and model registry. Lives here
 * (the Pi adapter) so the host-neutral engine never statically imports the Pi
 * approval UI.
 */
export const createPiExecutionHost = (
  context: ExtensionContext,
  getConfig: () => import("./config.js").FabricConfig,
  sessionApprovals: InstanceType<typeof import("./core/approval-controller.js").FabricSessionApprovals>,
  classifier?: import("./core/auto-approval-classifier.js").FabricAutoApprovalClassifier,
): FabricExecutionHost => ({
  cwd: context.cwd,
  payload: context,
  defaultAgentRunner: getConfig().agents.runner,
  ...
  createApprover(recordAutoDecision) {
...
export const createFabricExecTool = (
  ...
  defineTool({
    ...
    parameters: fabricExecInputSchema,
    ...
    async execute(toolCallId, params, signal, onUpdate, context) {
      ...
      const result = await state.execution.execute({
        ...
        host: createPiExecutionHost(
          context,
          () => state.config,
          state.execution.sessionApprovals,
          state.execution.autoApprovalClassifier,
        ),
