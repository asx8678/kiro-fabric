 ...
 */
export type FabricCoreProjectionNamespace = "pi" | "k";

const canonicalCoreProjectionRef = (
  ref: string,
  namespace: FabricCoreProjectionNamespace,
): string => namespace === "k" && ref.startsWith("k.") ? `pi.${ref.slice(2)}` : ref;

export const projectFabricAuditArgs = (
  ref: string,
  args: Record<string, unknown>,
  coreToolNamespace: FabricCoreProjectionNamespace = "pi",
): FabricAuditProjection => {
 ...
 * the core write tool. No provider details or output text accompany that flag.
 */
export const projectFabricAuditResult = (
  ...
  if (canonicalCoreProjectionRef(ref, coreToolNamespace) !== "pi.write") return undefined;
  const details =
