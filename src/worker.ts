...
type KiroWorkerModule = typeof import("./kiro/acp-worker.js");
type SteerFileModule = typeof import("./agents/steer-file.js");
...
const loadKiroWorker = async (): Promise<KiroWorkerModule> => {
  if (!import.meta.url.endsWith(".ts")) return import("./kiro/acp-worker.js");
  const sourceModulePath = "./kiro/acp-worker.ts";
  return import(sourceModulePath) as Promise<KiroWorkerModule>;
};

const loadSteerFile = async (): Promise<SteerFileModule> => {
  if (!import.meta.url.endsWith(".ts")) return import("./agents/steer-file.js");
  const sourceModulePath = "./agents/steer-file.ts";
  return import(sourceModulePath) as Promise<SteerFileModule>;
};
...
const main = async (): Promise<void> => {
  ...
  if (options.runner === "kiro") {
    const kiro = await loadKiroWorker();
    type KiroWorkerOptions = Parameters<typeof kiro.runKiroWorker>[0];
    type KiroWorkerHelpers = Parameters<typeof kiro.runKiroWorker>[1];
    // This is the compatibility bridge from the legacy multi-runner worker.
    // The Kiro worker itself receives a literal Kiro-only contract and cannot
    // dispatch Pi, Claude, or Veda.
    const kiroOptions = { ...options, runner: "kiro" as const } as KiroWorkerOptions;
    const helpers: KiroWorkerHelpers = {
      createRunningRecord: loadedRunRecordHelpers.createRunningRecord as unknown as KiroWorkerHelpers["createRunningRecord"],
      writeRunRecord: loadedRunRecordHelpers.writeRunRecord as unknown as KiroWorkerHelpers["writeRunRecord"],
      updateRunRecord: loadedRunRecordHelpers.updateRunRecord as unknown as KiroWorkerHelpers["updateRunRecord"],
      latestRunText: loadedRunRecordHelpers.latestRunText as unknown as KiroWorkerHelpers["latestRunText"],
    };
    const record = await kiro.runKiroWorker(kiroOptions, helpers);
    crashContext = { statusFile: options.statusFile, record };
    process.exitCode = record.status === "completed" ? 0 : 1;
    return;
  }
  const { createSteerReadState, readSteerLines } = await loadSteerFile();
  ...
  const child = spawnCli(childBinary, childArguments, {
    ...
    env: {
      ...
      ...(options.mainAgentId ? { KIRO_FABRIC_MAIN_AGENT_ID: options.mainAgentId } : {}),
      KIRO_FABRIC_GRANTED_RISKS: options.grantedRisks.join(","),
      ...
      ...(options.ownerIdentityId
        ? { KIRO_FABRIC_OWNER_IDENTITY_ID: options.ownerIdentityId }
        : {}),
      ...(options.runRoot ? { KIRO_FABRIC_RUN_ROOT: options.runRoot } : {}),
    },
  ...
  const enforceTokenLimit = (): void => {
    ...
    if (record.usage.availability === "unavailable") return;
    const total =
      record.usage.input +
      record.usage.output +
  ...
  const processEvent = (line: string): void => {
    if (process.env.KIRO_FABRIC_INJECT_CRASH === "stream") throw new Error("simulated stream crash");
    if (!line.trim()) return;
    appendLog(`${line}\n`);
  ...
  const steerState = createSteerReadState();
  const pollSteer = (): void => {
    if (!options.steerFile || terminalStatus) return;
    for (const line of readSteerLines(options.steerFile, steerState)) {
  ...
  if (process.env.KIRO_FABRIC_INJECT_CRASH === "close") throw new Error("simulated close crash");
  if (options.runner === "veda") {
