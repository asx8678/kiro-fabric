...
export function listPromptIds(): string[] {
  return readdirSync(promptDirectory)
...
export function loadPrompt(id: string): string {
  ...
  try {
    return readFileSync(path.join(promptDirectory, `${id}.md`), "utf8");
  } catch {
    throw new Error(`Unknown prompt: ${id}. Available: ${listPromptIds().join(", ")}`);
  }
}
...
export function renderPrompt(id: string, variables: Record<string, string> = {}): string {
  ...
  if (missing.length > 0)
    throw new Error(`Missing prompt variables for ${id}: ${missing.join(", ")}`);
