...
const KIRO_ONLY_GUEST_TYPE_DECLARATIONS = `
interface KiroFabricAgentRequest {
  task: string;
  name?: string;
  runner?: "kiro";
  ...
  tools?: string[];
  timeoutMs?: number;
  /** Managed children run at the Kiro project root. */
  cwd?: string;
  schema?: Record<string, unknown>;
  /** Bounded semantic context packet; not a native Kiro transcript transfer. */
  context?: {
    objective?: string;
    facts?: string[];
...
interface KiroFabricAgentsApi {
  run(args: KiroFabricAgentRequest): Promise<FabricAgentResult>;
  spawn(args: KiroFabricAgentRequest): Promise<FabricAgentHandle>;
  ...
  models(args?: { runner?: "kiro"; refresh?: boolean }): Promise<FabricModelInfo[]>;
  stop(args: { id: string }): Promise<FabricAgentResult>;
  cleanup(args: { id: string }): Promise<{ cleaned: boolean }>;
  steer(args: { id: string; message: string; data?: unknown }): Promise<{ queued: true; messageId: string }>;
  followUp(args: { id: string; message: string; data?: unknown }): Promise<{ queued: true; messageId: string }>;
  ...
  log(args: { id: string; lines?: number; before?: number }): Promise<FabricAgentLog>;
}
interface KiroToolsApi extends PiToolsApi {
  readArtifact(args: { id: string; offset?: number; limit?: number }): Promise<{
    id: string;
    text: string;
    offset: number;
    nextOffset: number;
...
interface KiroProjectMemoryEntry<T = unknown> {
  namespace: string;
  key: string;
  value: T;
  bytes: number;
...
interface KiroProjectMemoryApi {
  get<T = unknown>(args: { key: string }): Promise<KiroProjectMemoryEntry<T> | null>;
  set<T = unknown>(args: { key: string; value: T }): Promise<KiroProjectMemoryEntry<T>>;
  search<T = unknown>(args: { query: string; limit?: number }): Promise<KiroProjectMemoryEntry<T>[]>;
  index(): Promise<Array<{ key: string; bytes: number; updatedAt: string }>>;
}
`;
...
type FabricAgentRunner = "pi" | "claude" | "veda" | "kiro";
...
interface GlobalActorDefinition extends FabricActorRequestBase {
  id: string;
  createdAt: number;
  updatedAt: number;
  events: FabricActorHostEvent[];
...
interface FabricAgentsApi {
  ...
  clearMessages(args: FabricAgentTargetArgs): Promise<FabricActorInfo>;
  remove(args: { id: string; scope?: "project" | "global" }): Promise<{ removed: boolean }>;
  import(args: { id?: string; name?: string; as?: string }): Promise<FabricActorInfo>;
  export(args: { id: string; overwrite?: boolean }): Promise<GlobalActorDefinition>;
  log(args: {
    id: string;
...
export interface FabricGuestDeclarationOptions {
   ...
   * The block is applied only to the full-code core-tools declaration.
   */
  coreOverrides?: string;
  /** Model-facing core coding-tool global. Shared Pi defaults to `pi`. */
  coreToolNamespace?: "pi" | "k";
  /** Omit helpers that require child-agent usage accounting. */
...
export const guestTypeDeclarations = (
  ...
  if (options.coreToolNamespace === "k") {
    result += KIRO_ONLY_GUEST_TYPE_DECLARATIONS;
    result = result.replace(
      globalDeclarationLine("pi"),
      (declaration) => declaration
        .replace("declare const pi:", "declare const k:")
        .replace("PiToolsApi", "KiroToolsApi"),
    );
    result = result.replace(
      globalDeclarationLine("memory"),
      "declare const memory: KiroProjectMemoryApi;\n",
    );
    result = result.replace(
      globalDeclarationLine("agents"),
      "declare const agents: KiroFabricAgentsApi;\n",
    );
    result = result.replace(
      globalDeclarationLine("mcp"),
      "declare const mcp: KiroFabricMcpApi;\n",
    );
  }
  if (options.agentBackedOrchestration === false) {
    result = result
      .replace(
        /^  agent<T = string>\(prompt: string, options\?: FabricWorkflowAgentOptions\): Promise<T>;\n/m,
        "",
