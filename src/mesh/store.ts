...
export class MeshCompareAndSwapError extends Error {
  constructor(
    readonly key: string,
...
export class MeshStore {
  ...
  async publishBatch(
    ...
    return this.#withLock(() => {
      ...
      const events: MeshEvent[] = prepared.map((part, index) => ({ ...part, sequence: first + index }));
  ...
  #prepareEvent(input: {
    ...
    const data: MeshEvent = {
      id: randomUUID(),
      sequence: 0, // assigned under the lock in publishBatch
      topic: input.topic,
