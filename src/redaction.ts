const patterns: Array<{
  ...
  {
    className: "private-key-json",
    regex:
      /-----BEGIN (?:RSA |EC |OPENSSH |DSA |PGP )?PRIVATE KEY-----(?:[A-Za-z0-9+/=]|\\n|\\\\n|\s)*-----END (?:RSA |EC |OPENSSH |DSA |PGP )?PRIVATE KEY-----/g,
    replace: (_m, p) => p,
  },
  {
    className: "authorization",
    regex: /(authorization\s*:\s*(?:bearer|basic)\s+)([^\s,"']{12,})/gi,
    replace: (_m, p, g) => `${g[0]}${p}`,
  },
  {
    className: "credential",
    regex:
      /((?:api[_-]?key|access[_-]?token|auth[_-]?token|client[_-]?secret|password|passwd)\s*[=:]\s*["']?)([^\s,"';}]{8,})/gi,
    replace: (_m, p, g) => `${g[0]}${p}`,
  },
  ...
  },
  {
    ...
    regex: /data:(?:image|audio|video)\/[a-z0-9.+-]+;base64,[A-Za-z0-9+/=\r\n]{256,}/gi,
    replace: (_m, p) => p,
  },
  // High-entropy base64 segments (64+ chars of pure base64 not preceded by common safe prefixes)
  {
    className: "high-entropy-base64",
    regex: /(?<![A-Za-z0-9_/.-])(?:[A-Za-z0-9+/]{16,}={0,2}){1}(?=["\s,}\])]|$)/gm,
    replace: (match, p) => {
    ...
    for (const pattern of patterns)
      output = output.replace(pattern.regex, (...args: unknown[]) => {
        ...
        const result = pattern.replace(
          String(args[0]),
          `[REDACTED:${pattern.className}:${ordinal}]`,
          groups,
        );
  ...
  value<T>(value: T): T {
    ...
    if (value && typeof value === "object")
      return Object.fromEntries(
        Object.entries(value).map(([key, item]) => [key, this.value(item)]),
      ) as T;
    return value;
  }
}
...
export function redactSensitive(text: string): string {
  return new RequestRedactor().redact(text);
}
