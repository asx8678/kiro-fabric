...
export interface FabricAgentConfig {
  ...
  /** Export store root override; KIRO_FABRIC_AGENT_DIR wins. Empty = ~/.kiro-fabric/agent. */
...
const runnerValue = (value: unknown, fallback: FabricAgentRunner): FabricAgentRunner =>
  parseFabricAgentRunner(value, fallback);
...
/** Read configuration without persisting migrations or mutating process state. */
export const inspectFabricConfig = (options: {
  cwd: string;
  agentDir: string;
  projectTrusted: boolean;
}): FabricConfig =>
  resolveFabricConfig(options, options.projectTrusted, false);
...
export const saveFabricConfig = (
  ...
  const normalized = migrateFabricConfigDocument(merged);
  writeJsonAtomic(targetPath, normalized.document, input?.source);
