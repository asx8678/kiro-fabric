...
export class DashboardDetailRenderer {
  ...
  private detailLines(entity: Entity, width: number, now: number, cwd: string): string[] {
    ...
    if (entity.kind === "main") {
      ...
      field("Usage", agent.usage
        ? isAvailableUsage(agent.usage)
          ? `${formatTokens(agent.usage.input + agent.usage.output)} tokens · ${agent.toolCalls ?? 0} tools · ${agent.turns ?? 0} turns · $${agent.usage.cost.toFixed(4)}`
          : "unavailable"
        : undefined);
      ...
      field(
        "Usage",
        actor.worker?.usage
          ? isAvailableUsage(actor.worker.usage)
