...
export async function initHighlighting(theme: string, syntaxEnabled = true): Promise<void> {
  ...
  try {
    ...
    // Resolve the theme object from kiro-fabric's own module graph and hand
    ...
    initializingTheme = undefined;
    console.warn("[kiro-fabric] Shiki failed to initialize; previews will be plain text.", error);
    highlighter?.dispose();
    highlighter = undefined;
    readyTheme = undefined;
    highlighterGeneration++;
