...
const RUNNERS = ["pi", "claude", "veda", "kiro"] as const;
...
export const buildFabricSettingsItems = (
  ...
  const envFullCode = process.env.KIRO_FABRIC_FULL_CODE_MODE;
  const fullCodeDescription = envFullCode
    ? "Fabric owns Pi core tools (read, bash, edit, write, grep, find, ls) via fabric_exec. Currently overridden by the KIRO_FABRIC_FULL_CODE_MODE environment variable."
      ...
      submenu: sectionSubmenu(
        ...
        [
          ...
          setting(
            ...
            "Max source bytes",
            formatBytes(config.executor.maxSourceBytes),
            {
              description: "UTF-8 byte cap enforced before fabric_exec type checking or runtime startup.",
              submenu: numericSubmenu(
                theme,
                [64 * 1024, 128 * 1024, 256 * 1024, 512 * 1024],
                formatBytes,
                "Max source bytes",
                "UTF-8 byte cap enforced before fabric_exec type checking or runtime startup.",
              ),
            },
          ),
