...
const ASSISTANT_CONTEXT_LINES = PAGE_LINES * 4;
const MAX_ASSISTANT_CONTEXT_BYTES = 32 * 1024 * 1024;
...
const isAssistantDelta = (event: Record<string, unknown>): boolean => {
  const message = recordOf(event.message);
  return event.type === "message_update" && event.delta === true && message?.role === "assistant";
};

/** Index at which the current assistant reconstruction can safely begin. */
const assistantBoundary = (events: Array<Record<string, unknown>>): number | undefined => {
  for (let index = events.length - 1; index >= 0; index--) {
    const event = events[index]!;
    const message = recordOf(event.message);
    if (event.type === "message_start" && message?.role === "assistant") return index;
    if (
      event.type === "message_update" &&
      event.delta !== true &&
      message?.role === "assistant"
    ) return index;
    if (event.type === "message_end" || event.type === "turn_end") return index + 1;
  }
  return undefined;
};
...
export class AgentTranscriptReader {
  ...
  #stateForPage(
    ...
    const firstAssistantEvent = events.find((event) => {
      const message = recordOf(event.message);
      return message?.role === "assistant" &&
        (event.type === "message_start" || event.type === "message_update" || event.type === "message_end");
    });
    const assistantContext: Array<Record<string, unknown>> = [];
    if (firstAssistantEvent && isAssistantDelta(firstAssistantEvent) && pageStart > 0) {
      let cursor = pageStart;
      let bytesRead = 0;
      while (cursor > 0 && bytesRead < MAX_ASSISTANT_CONTEXT_BYTES) {
        const contextPage = readJsonlPageFromDescriptor(
          descriptor,
          ASSISTANT_CONTEXT_LINES,
          cursor,
          stat.size,
          Math.min(MAX_PAGE_BYTES, MAX_ASSISTANT_CONTEXT_BYTES - bytesRead),
        );
        const contextEvents = parsedEvents(contextPage.lines);
        const boundary = assistantBoundary(contextEvents);
        if (boundary !== undefined) {
          assistantContext.unshift(...contextEvents.slice(boundary));
          break;
        }
        assistantContext.unshift(...contextEvents);
        const nextCursor = contextPage.lines[0]?.offset ?? 0;
        if (nextCursor >= cursor) break;
        bytesRead += cursor - nextCursor;
        cursor = nextCursor;
        if (!contextPage.hasMore) break;
      }
    }
    const reconstructionEvents = [...assistantContext, ...events];
    const missingStarts = missingToolStartIds(reconstructionEvents);
    ...
    accumulator.append([...lifecycleContext, ...reconstructionEvents]);
    const transcript = {
