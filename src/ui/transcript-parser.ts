...
export class TranscriptAccumulator {
  ...
  #append(event: Record<string, unknown>): void {
    ...
    if (event.type === "message_start" || event.type === "message_update") {
      ...
      // message_update historically carried cumulative snapshots. Kiro's
      // bounded stream uses explicit deltas so a live reader and a replay of a
      // failed run can reconstruct all chunks without quadratic log growth.
      const isDelta = event.type === "message_update" && event.delta === true;
      const text = terminalSafe(contentText(message.content), !isDelta);
      if (!text) return;
      if (!this.#assistant) {
        this.#assistant = {
          id,
          kind: "assistant",
          label: "Agent",
          text: clip(text, MAX_TRANSCRIPT_MESSAGE_CHARS, !isDelta),
          status: "running",
        };
        this.entries.push(this.#assistant);
      } else {
        this.#assistant.text = clip(
          isDelta ? `${this.#assistant.text ?? ""}${text}` : text,
          MAX_TRANSCRIPT_MESSAGE_CHARS,
          !isDelta,
        );
    ...
    if (event.type === "agent_end" || event.type === "agent_settled") {
      this.#finishAssistant(event.status === "completed" ? "completed" : "failed");
      return;
    }
