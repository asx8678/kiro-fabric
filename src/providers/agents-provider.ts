...
import { isAvailableUsage } from "../agents/types.js";
import type {
  AgentHandleInfo,
  AgentRunRecord,
  AgentRunRequest,
...
const runRequest = (
  ...
  if (args.runner !== undefined && !isFabricAgentRunner(args.runner)) {
    throw new Error(`Unsupported Fabric agent runner: ${String(args.runner)}`);
  }
  const runner = isFabricAgentRunner(args.runner) ? args.runner : manager.config.runner;
...
const actorRequest = (
  ...
  if (runner === "veda") {
    throw new Error(
      'The Veda runner does not support persistent actors: Veda executes one headless prompt per invocation. Use a Pi, Claude, or Kiro actor, or agents.run({ runner: "veda" }).',
    );
  }
  const inheritedModel =
...
export const collectAgentToolPreviewNodes = (
  ...
  for (const record of records) {
    ...
    nodes.push({
      ...
      ...(isFabricAgentRunner(record.runner) ? { runner: record.runner } : {}),
...
const waitWithProgress = async (
  ...
  try {
    const settled = await waitForResultWithProgress(result, () => {
      ...
      if ("usage" in status && isAvailableUsage(status.usage)) {
        context.activity?.({
          type: "metrics",
    ...
    if (isAvailableUsage(settled.usage)) {
      context.activity?.({
        type: "metrics",
    ...
    switch (actionName) {
      ...
      case "status": {
        ...
        if (this.mainAgent.matches(id)) {
          if (this.mainAgent.local) return this.mainAgent.info(context.extensionContext as ExtensionContext);
      ...
      case "main":
        return this.mainAgent.info(context.extensionContext as ExtensionContext);
      case "peers":
      ...
      case "models": {
        ...
        const runner = isFabricAgentRunner(args.runner)
          ? args.runner
          : this.manager.config.runner;
        if (runner === "veda" || runner === "kiro") {
          // Veda and Kiro accept caller-supplied model strings; they do not
          // expose a Fabric-owned discovery list yet.
          return [];
        }
        if (runner === "claude") {
