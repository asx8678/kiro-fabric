...
const runProperties = {
  ...
  model: {
    type: "string",
    description: "Pi provider/id, Claude runtime value, Veda backend model/alias, or Kiro v3 ACP session model ID.",
  },
  persona: {
    type: "string",
  ...
  worktree: {
    type: "boolean",
    description: "Create an isolated Git worktree. Unsupported for the Kiro runner because the managed profile binds MCP to the project root.",
  },
...
export const AGENTS_ACTION_DESCRIPTORS: FabricActionDescriptor[] = [
  {
    name: "run",
    description: "Run a child agent through Pi, Claude Code, Veda, or Kiro and wait for its final result",
  ...
  },
  {
    name: "spawn",
    description:
      "Start a child agent through Pi, Claude Code, Veda, or Kiro and return a handle immediately. Detached runs send Main a follow-up on terminal completion when agents.notifyOnComplete is enabled; use wait when this Fabric program needs the result and status only for progress inspection.",
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        runner: { type: "string", enum: ["pi", "claude", "veda", "kiro"] },
  ...
  },
  {
    name: "compact",
    description:
      "Request an advisory compaction of a running Pi-runner child agent's context at its next safe boundary (between its own turns), preserving the child's accumulated context. Rejected for Claude, Veda, and Kiro children. The child pi core applies the compaction; Fabric only forwards the intent.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string" },
