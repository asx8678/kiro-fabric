...
const unambiguousTools = (tools: ServerToolInfo[]): ServerToolInfo[] => {
...
export class McpProvider implements FabricProvider {
  ...
  sliceDescriptors(): FabricActionDescriptor[] {
    ...
    for (const [server, entry] of this.#servers) {
      ...
      for (const tool of unambiguousTools(entry.tools)) {
        descriptors.push(this.#toolDescriptor(server, tool));
      }
    }
    return descriptors;
  }
  ...
  async #resolveKnownServer(
    ...
    if (aliases.length > 0) return aliases.length === 1 ? aliases[0] : undefined;
    // Preserve explicit raw-name management calls when the raw value itself is
    // not an identifier-safe alias. Identifier-safe collisions always fail
    // closed because the generated mcp.<server> ref cannot disambiguate them.
    return servers.includes(requested) ? requested : undefined;
  }
  ...
  async #listLegacy(
    ...
    const runtime = await this.#getRuntime();
    const resolvedNamespace = request.namespace
      ? this.#resolveServerName(runtime, request.namespace)
      : undefined;
    const configuredServers = runtime.listServers();
    const visibleServers = unambiguousRawNames(configuredServers);
    const servers = request.namespace
      ? (resolvedNamespace === undefined ? [] : [resolvedNamespace])
      : configuredServers.filter((server) => visibleServers.has(server));
