...
export interface Diagnostic { code: number; category: "error" | "warning"; message: string; line: number; column: number; sourceLine?: string; hint?: string }
...
export function checkProgram(body: string): CheckResult {
 ...
 for (const d of ts.getPreEmitDiagnostics(program)) { if (d.file?.fileName !== fileName) continue; const pos = d.file.getLineAndCharacterOfPosition(d.start ?? 0); const line = Math.max(1, pos.line); const sourceLine = body.split("\n")[line - 1]; const msg = ts.flattenDiagnosticMessageText(d.messageText, "\n"); const hint = diagnosticHint(d.code, msg, sourceLine); diagnostics.push({ code: d.code, category: d.category === ts.DiagnosticCategory.Warning ? "warning" : "error", message: msg, line, column: pos.character + 1, ...(sourceLine === undefined ? {} : { sourceLine }), ...(hint ? { hint } : {}) }); }
