...
export interface FabricMainAgentDeliveryRequest {
  ...
  triggerTurn?: boolean;
  data?: unknown;
  /** Stable durable-mailbox key; repeated delivery is acknowledged, not sent. */
...
export interface FabricMainAgentTarget {
  ...
  acknowledgeDelivery?(idempotencyKey: string): void;
}
...
export const resolveFabricIdentity = (
  ...
  const actorId = environment.KIRO_FABRIC_ACTOR_ID?.trim();
  const parentAgentId = environment.KIRO_FABRIC_PARENT_RUN?.trim();
  const identity: MeshIdentity = actorId
    ? {
        id: actorId,
        name: environment.KIRO_FABRIC_ACTOR_NAME?.trim() || actorId.slice(0, 8),
        kind: "actor",
        sessionId,
      }
    : parentAgentId
      ? {
          id: parentAgentId,
          name: environment.KIRO_FABRIC_AGENT_NAME?.trim() || parentAgentId.slice(0, 8),
          kind: "agent",
          sessionId,
        }
      : { id: `session:${sessionId}`, name: "main", kind: "main", sessionId };
  const inheritedMainAgentId = environment.KIRO_FABRIC_MAIN_AGENT_ID?.trim();
  return {
    identity,
    mainAgentId:
...
export class MainAgentController implements FabricMainAgentTarget {
  ...
  readonly #pendingDeliveryIds = new Map<string, string>();
  ...
  deliverAgent(request: FabricMainAgentDeliveryRequest): FabricAgentMessageResult {
    ...
    const duplicateId = idempotencyKey
      ? this.#pendingDeliveryIds.get(idempotencyKey)
      : undefined;
    if (duplicateId) {
      return { queued: true, messageId: duplicateId, routed: "main", acknowledged: true };
    }
    const messageId = randomUUID();
    if (idempotencyKey) this.#pendingDeliveryIds.set(idempotencyKey, messageId);
    ...
    try {
      ...
      if (idempotencyKey) this.#pendingDeliveryIds.delete(idempotencyKey);
      throw error;
    }
    return { queued: true, messageId, routed: "main" };
  }
  ...
  acknowledgeDelivery(idempotencyKey: string): void {
    this.#pendingDeliveryIds.delete(idempotencyKey);
  }
}
