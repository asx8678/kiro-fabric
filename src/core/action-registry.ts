...
/**
 * Prefix kiro-fabric prepends to every nested tool-call id it generates inside a
...
export class ActionRegistry {
  ...
  readonly #guestTypeCache = new Map<string, Promise<FabricGuestTypeSources>>();
  /** Search results keyed by epoch + view digest + normalized query + limit. */
  readonly #searchCache = new Map<string, ResolvedFabricAction[]>();
  ...
  #viewKey(context: FabricInvocationContext): string {
  ...
  async search(
    ...
    // Cache fresh arrays; callers never receive a shared live snapshot.
  ...
  #parseRef(
    ...
    const pinned =
      view && Object.hasOwn(view.bindings, ref) ? view.bindings[ref] : undefined;
    if (view && !pinned) {
...
/** Deep-clone cached guest type sources so callers never mutate a shared snapshot. */
const cloneGuestTypeSources = (sources: FabricGuestTypeSources): FabricGuestTypeSources => {
  const out: FabricGuestTypeSources = {};
