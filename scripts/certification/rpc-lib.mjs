...
export const benchmarkGate = (env = process.env) => {
  const reasons = [];
  if (env.KIRO_FABRIC_REAL_RESUME !== "1") reasons.push("KIRO_FABRIC_REAL_RESUME must equal 1");
  if (!env.KIRO_FABRIC_BENCH_MODEL) reasons.push("KIRO_FABRIC_BENCH_MODEL is required");
  if (!env.KIRO_FABRIC_BENCH_PROVIDER) reasons.push("KIRO_FABRIC_BENCH_PROVIDER is required");
  const keyVariable = env.KIRO_FABRIC_BENCH_KEY_ENV;
  if (!keyVariable) reasons.push("KIRO_FABRIC_BENCH_KEY_ENV is required");
  ...
  const repeats = positiveInteger(env.KIRO_FABRIC_BENCH_REPEATS);
  if (repeats === null) reasons.push("KIRO_FABRIC_BENCH_REPEATS must be a positive integer");
  const maxUsd = positiveNumber(env.KIRO_FABRIC_BENCH_MAX_USD);
  if (maxUsd === null) reasons.push("KIRO_FABRIC_BENCH_MAX_USD must be a positive number");
  return {
    enabled: reasons.length === 0,
    reasons,
    config: {
      model: env.KIRO_FABRIC_BENCH_MODEL ?? null,
      provider: env.KIRO_FABRIC_BENCH_PROVIDER ?? null,
      keyVariable: keyVariable ?? null,
      repeats: repeats ?? 0,
      maxUsd: maxUsd ?? 0,
      seed: env.KIRO_FABRIC_BENCH_SEED ?? "kiro-fabric-resume-v1",
      piCommand: env.KIRO_FABRIC_PI_COMMAND ?? "pi",
