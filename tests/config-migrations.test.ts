...
const fixture = () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-migration-"));
  roots.push(root);
...
describe("Fabric configuration migrations", () => {
  ...
  it("accepts future config versions forward-compatibly without mutation", () => {
    const input = { configVersion: 5, agents: { runner: "future-runner" }, ui: { maxRows: 3 } };
    const result = migrateFabricConfigDocument(input);

    expect(result.forwardCompatible).toBe(true);
    expect(result.toVersion).toBeGreaterThanOrEqual(5);
    ...
    expect(input).toEqual({ configVersion: 5, agents: { runner: "future-runner" }, ui: { maxRows: 3 } });
  });
  ...
  it("migrates a v3 Kiro selection to v4 without changing its content", () => {
    const input = { configVersion: 3, agents: { runner: "kiro" } };
    const result = migrateFabricConfigDocument(input);
    expect(result).toMatchObject({
      fromVersion: 3,
      toVersion: 4,
    ...
    expect(result.document).toEqual({ configVersion: 4, agents: { runner: "kiro" } });
    expect(input).toEqual({ configVersion: 3, agents: { runner: "kiro" } });
  });

  it("accepts an already-current v4 Kiro document without rewriting it", () => {
    const paths = fixture();
    const current = JSON.stringify({ configVersion: 4, agents: { runner: "kiro" } }, null, 2) + "\n";
    fs.writeFileSync(paths.globalPath, current);
    const before = fs.statSync(paths.globalPath).mtimeMs;
    const config = loadFabricConfig({
      cwd: paths.cwd,
    ...
    expect(config.agents.runner).toBe("kiro");
    expect(fs.readFileSync(paths.globalPath, "utf8")).toBe(current);
    expect(fs.statSync(paths.globalPath).mtimeMs).toBe(before);
  });

  it("rejects an unknown runner in a current document before rewriting it", () => {
    const paths = fixture();
    const current = JSON.stringify({ configVersion: 4, agents: { runner: "future-runner" } });
    fs.writeFileSync(paths.globalPath, current);
    const before = fs.statSync(paths.globalPath).mtimeMs;
    expect(() =>
      loadFabricConfig({ cwd: paths.cwd, agentDir: paths.agentDir, projectTrusted: false }),
  ...
  it("accepts a newer schema before interpreting its runner", () => {
    const input = {
      configVersion: 5,
  ...
  it("validates global and project layers independently", () => {
    const paths = fixture();
    fs.writeFileSync(
      paths.globalPath,
      JSON.stringify({ configVersion: 4, agents: { runner: "future-runner" } }),
    );
    fs.writeFileSync(
      paths.projectPath,
      JSON.stringify({ configVersion: 4, agents: { runner: "kiro" } }),
    );
    expect(() =>
      loadFabricConfig({ cwd: paths.cwd, agentDir: paths.agentDir, projectTrusted: true }),
    ).toThrow(/Unsupported Fabric agent runner: future-runner/);
    expect(JSON.parse(fs.readFileSync(paths.globalPath, "utf8"))).toEqual({
      configVersion: 4,
      agents: { runner: "future-runner" },
