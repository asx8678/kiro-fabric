...
describe("ActorManager", () => {
  it.skipIf(process.platform === "win32")("uses event monitoring when fs.watch is available", async () => {
    const watcher = {
  ...
  it("uses polling when event monitoring is unavailable", async () => {
  ...
  it("resumes a Claude Code session after a persistent actor is restored", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-claude-actor-"));
    roots.push(root);
  ...
  it("restores project-scoped actors across different Pi sessions", async () => {
    ...
    // redefining actors.
    const scopeDir = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-actor-scope-"));
    roots.push(scopeDir);
