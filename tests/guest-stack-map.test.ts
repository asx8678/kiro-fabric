...
describe("remapGuestErrorText", () => {
  it("maps emitted frames back to user code lines", () => {
    ...
    const stack = `Error: boom\n    at __kiroFabricMain (kiro-fabric-guest.js:${line}:${column})`;
    expect(remapGuestErrorText(stack, stackMap)).toContain(
      "at __kiroFabricMain (guest code:3:",
    );
  });
  ...
  it("passes JSON.parse style errors through with only the guest frame remapped", () => {
    ...
    const stack = [
      ...
      "    at <input>:1:5",
      "    at parse (native)",
      `    at __kiroFabricMain (kiro-fabric-guest.js:${line}:1)`,
    ].join("\n");
    ...
    expect(remapped).not.toContain("kiro-fabric-guest.js");
  });

  it("returns text unchanged when no stack map is available", () => {
    const text = "    at __kiroFabricMain (kiro-fabric-guest.js:11:29)";
  ...
  it("relabels frames on runtime-appended driver lines", () => {
    ...
    const text = `    at <eval> (kiro-fabric-guest.js:${guestLines + 1}:29)`;
  ...
  it("keeps unmapped frames intact when no guest line count is given", () => {
    ...
    const text = "    at f (kiro-fabric-guest.js:999:1)";
