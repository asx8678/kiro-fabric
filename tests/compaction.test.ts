...
describe("Pi custom_message compaction", () => {
  it("normalizes hidden and visible context structurally while excluding plain custom entries", () => {
    ...
    expect(events).toMatchObject([
      { customType: "kiro-fabric-actor", text: "actor completed <typed>", display: true },
  ...
  it("includes actor and agent completions in cumulative summaries deterministically", () => {
    ...
    const session = buildSession(
      user("Original task"),
      customMessage("kiro-fabric-actor", "Actor says: keep ACTOR_FACT_17", true, {
        actor: { id: "actor-17" },
        message: { status: "completed" },
      }),
      assistant(textPart("actor received")),
      customMessage("kiro-fabric-agent-complete", "Fabric agent abc completed: AGENT_FACT_23", false, {
        id: "agent-23",
        status: "completed",
      }),
    ...
    expect(first.compaction.summary).toContain('custom "kiro-fabric-actor" (visible)');
    expect(first.compaction.summary).toContain("ACTOR_FACT_17");
    expect(first.compaction.summary).toContain('custom "kiro-fabric-agent-complete" (hidden)');
...
describe("compaction cumulative endurance", () => {
  it("retains raw cumulative truth through 100 parent-linked compaction cycles", () => {
    ...
    appendLinked(
      ...
      customMessage("kiro-fabric-actor", "Preserve CUSTOM_CYCLE_FACT_31", false, { status: "completed" }),
