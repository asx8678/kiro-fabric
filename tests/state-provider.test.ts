...
const createStore = (maxReadEvents = 100): MeshStore => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-state-"));
  roots.push(root);
...
describe("StateStore", () => {
  ...
  it("revokes a successful certificate when the latest verification fails", async () => {
    const mesh = createStore();
    const store = new StateStore(mesh);
    const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-revoke-"));
    roots.push(project);
  ...
  it("revokes the durable current certificate even when violation publication fails", async () => {
    const mesh = createStore();
    const store = new StateStore(mesh);
    const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-revoke-reporting-"));
    roots.push(project);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const mesh = createStore();
      const store = new StateStore(mesh);
      const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-process-group-"));
      roots.push(project);
  ...
  it("rejects a proposal on ledger failure and rolls back prior ledger writes", async () => {
    const mesh = createStore();
    const store = new StateStore(mesh);
    const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-ledger-failure-"));
    roots.push(project);
  ...
  it("restores the previous head and ledgers when the commit marker fails", async () => {
    const mesh = createStore();
    const store = new StateStore(mesh);
    const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-commit-failure-"));
    roots.push(project);
    fs.mkdirSync(path.join(project, "src"));
  ...
  it("quarantines a rejected proposal when CAS restoration fails", async () => {
    const mesh = createStore();
    const store = new StateStore(mesh);
    const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-quarantine-"));
    roots.push(project);
    fs.mkdirSync(path.join(project, "src"));
  ...
  it("retains the committed current head, synthesized record, certificate, and transitionability with maxReadEvents=5", async () => {
    ...
    const store = new StateStore(mesh);
    const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-retained-head-"));
    roots.push(project);
    fs.mkdirSync(path.join(project, "src"));
  ...
  it("durably revokes a current certificate after failure and event aging", async () => {
    const mesh = createStore(5);
    const store = new StateStore(mesh);
    const project = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-retained-revoke-"));
    roots.push(project);
