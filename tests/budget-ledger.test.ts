...
describe("budget ledger", () => {
  ...
  it("treats a non-positive budget as inactive", () => {
    process.env.KIRO_FABRIC_BUDGET = "0";
    process.env.KIRO_FABRIC_BUDGET_FILE = "/tmp/ignored";
    process.env.KIRO_FABRIC_BUDGET_ID = "x";
    try {
      expect(activeBudgetState()).toBeUndefined();
    } finally {
      clearOwnedBudgetEnv();
  ...
  it("returns zero for a missing ledger file", () => {
    expect(readBudgetLedger(path.join(os.tmpdir(), "kiro-fabric-missing-cost.jsonl"))).toEqual({
      cost: 0,
      tokens: 0,
    });
  });
