...
input.on("line", (line) => {
  ...
  if (task.includes("REPORT_FABRIC_IDENTITY")) {
    finishAttempt(
      successMessage(
        JSON.stringify({
          mainAgentId: process.env.KIRO_FABRIC_MAIN_AGENT_ID,
          parentRun: process.env.KIRO_FABRIC_PARENT_RUN,
          agentName: process.env.KIRO_FABRIC_AGENT_NAME,
        }),
      ),
      false,
    );
  ...
  const value = {
    action: "message",
    message: `validated actor response:${process.env.KIRO_FABRIC_FULL_CODE_MODE ?? "missing"}`,
  };
