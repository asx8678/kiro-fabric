...
const setup = () => {
  const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-global-actors-"));
  dirs.push(agentDir);
...
describe("GlobalActorRegistry", () => {
  ...
  it("round-trips a Kiro template and does not coerce unknown runners to Pi", () => {
    const { agentDir, registry } = setup();
    const created = registry.create({ ...baseRequest, name: "kiro-reviewer", runner: "kiro" });
    expect(created.runner).toBe("kiro");
    const saved = JSON.parse(
      fs.readFileSync(path.join(agentDir, "fabric", "actors", "global-actors.json"), "utf8"),
    ) as { format: number; actors: Array<{ runner?: string }> };
    expect(saved.format).toBe(2);
    expect(saved.actors[0]?.runner).toBe("kiro");
    expect(new GlobalActorRegistry(agentDir, 64 * 1024).resolve("kiro-reviewer")?.runner).toBe("kiro");
    ...
    const mixed = new GlobalActorRegistry(agentDir, 64 * 1024);
    expect(mixed.resolve("legacy-pi")?.runner).toBe("pi");
    expect(mixed.resolve("future-runner")).toBeUndefined();
  });
