...
const serviceFor = (
...
): { service: FabricExecutionService; host: ReturnType<typeof testHost> } => {
  ...
  return { service, host: testHost(process.cwd(), () => service.config) };
};
...
const serviceForRegistry = (
  ...
  config.approvals.read = "allow";
  const service = new FabricExecutionService(registry, config);
  return { service, host: testHost(cwd, () => service.config) };
};
...
describe("Fabric execution trace V1", () => {
  ...
  it("records all discovery paths in issue order without queries or results", async () => {
    ...
    const { service, host } = serviceForRegistry(registry);
    // modelRegistry surfaced via host seam: swap in a models-bearing host
    host.listModels = () => [{ provider: "model-provider-secret", id: "model-id-secret", name: "model-name-secret", key: "model-provider-secret/model-id-secret" }];
    const result = await execute(
      service,
      host,
      `
...
`,
  ...
  it("records a typed failure for every discovery operation", async () => {
    ...
    const providersResult = await execute(providers.service, providers.host, "return tools.providers();");

    const modelsRegistry = new ActionRegistry();
    const models = serviceForRegistry(modelsRegistry);
    models.host.listModels = () => {
      throw new Error("models-failure-secret");
    };
    const modelsResult = await execute(models.service, models.host, "return tools.models();");
    ...
    const catalogResult = await execute(catalog.service, catalog.host, 'return tools.catalog({ provider: "demo" });');
    const list = serviceFor(failingProvider);
    const listResult = await execute(list.service, list.host, 'return tools.list({ provider: "demo" });');
    const search = serviceFor(failingProvider);
    const searchResult = await execute(search.service, search.host, 'return tools.search({ query: "failure-query-secret" });');
...
`,
  ...
  it("records workflow validation failures before activity mutation", async () => {
    const { service, host } = serviceFor();
    const result = await execute(service, host, 'return workflow.phase("   ");');
...
`,
  ...
  it("seals unclosed workflow spans on timeout and abort", async () => {
    ...
    const timedResult = await execute(timed.service, timed.host, code);
  ...
  it.each<{
    ...
    const result = await execute(service, host, code);
  ...
  it("explains disabled providers in type errors and resolve failures", async () => {
    ...
    const typed = await execute(service, host, 'return memory.recall({ query: "x" });');
  ...
  it("returns a failed zero-call trace for type-check failure without source text", async () => {
    const { service, host } = serviceFor();
    const result = await execute(service, host, "return rawCodeSecretIdentifier;");
  ...
  it("is byte-stable when legacy random IDs and timings differ", async () => {
    ...
    const firstResult = await execute(first.service, first.host, code);
    await new Promise((resolve) => setTimeout(resolve, 5));
    const secondResult = await execute(second.service, second.host, code);
  ...
  it("strictly ignores malformed and unknown trace versions", () => {
    ...
    expect(readFabricExecutionTraceV1({ kind: "kiro-fabric.execution", version: 1 })).toBeUndefined();
