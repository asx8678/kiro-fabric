...
describe("FabricExecutionService", () => {
  it("rejects oversized UTF-8 source before TypeScript checking", async () => {
    ...
    const result = await new FabricExecutionService(new ActionRegistry(), config).execute({
      code: "é".repeat(17),
      signal: undefined,
      parentToolCallId: "oversized-source",
...
`,
  ...
  it.each(["quickjs", "node-process"] as const)(
    ...
    async (runtime) => {
      const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-prewalk-"));
      try {
        const registry = new ActionRegistry();
...
`,
  ...
  it("calls a Pi built-in from sandboxed TypeScript", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-execution-"));
    try {
...
`,
  ...
  it("keeps Pi core tools outside Fabric in orchestration-only mode", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-native-tools-"));
    try {
...
`,
  ...
  it("publishes declarative workflow activity for the dynamic TUI", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-activity-"));
    try {
...
`,
  ...
  it("audits auto approvals and accounts for classifier usage", async () => {
    ...
    const result = await service.execute({
      ...
      host: testHost(process.cwd(), () => service.config, service.autoApprovalClassifier),
      onPartial() {},
    });
...
describe("FabricExecutionService dynamic guest typing", () => {
  ...
  const setup = (providers: FabricProvider[]) => {
    ...
    const run = (code: string, parentToolCallId: string) =>
      service.execute({ code, signal: undefined, parentToolCallId, host, onPartial() {} });
    return { service, host, run };
  };
