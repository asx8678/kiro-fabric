...
describe.skipIf(!hasWorker)("agent worker launch under a bundled pi binary", () => {
  ...
  const originalOverride = process.env.KIRO_FABRIC_NODE_BINARY;
  const originalBehavior = process.env.FAKE_PI_BEHAVIOR;

  afterEach(async () => {
    process.execPath = originalExecPath;
    if (originalOverride === undefined) delete process.env.KIRO_FABRIC_NODE_BINARY;
    else process.env.KIRO_FABRIC_NODE_BINARY = originalOverride;
  ...
  it("resolves a JS runtime and completes a subagent when process.execPath is the pi binary", async () => {
    ...
    process.env.KIRO_FABRIC_NODE_BINARY = realNode;
    process.env.FAKE_PI_BEHAVIOR = "success";

    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-bundled-"));
    roots.push(root);
