...
describe("script runtime resolution", () => {
  ...
  it("falls back to KIRO_FABRIC_NODE_BINARY when execPath is the bundled pi binary", async () => {
    const execPath = "/usr/local/bin/pi";
    const override = "/opt/node-v22/bin/node";
    expect(
      resolveScriptRuntimeSync({ execPath, env: { KIRO_FABRIC_NODE_BINARY: override } }),
    ).toBe(override);
    expect(
      await resolveScriptRuntime({ execPath, env: { KIRO_FABRIC_NODE_BINARY: override } }),
    ).toBe(override);
  });
  ...
  it("builds the full spawn argv prefix through scriptSpawnArgs", async () => {
    const args = await scriptSpawnArgs(
      ...
      { execPath: "/usr/local/bin/pi", env: { KIRO_FABRIC_NODE_BINARY: "/opt/node" } },
    );
  ...
  it("throws a clear error when the bundled binary has no runtime and no override", () => {
    expect(() => resolveScriptRuntimeSync({ execPath: "/usr/local/bin/pi", env: {} })).toThrow(
      /requires a Node\.js or Bun runtime|KIRO_FABRIC_NODE_BINARY/,
    );
  });
