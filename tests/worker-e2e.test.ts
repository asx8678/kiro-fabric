...
// inherited KIRO_FABRIC_* variables.
const fabricEnvKeys = [
  "KIRO_FABRIC_DEPTH",
  "KIRO_FABRIC_BUDGET",
...
describe.skipIf(!hasWorker)("AgentManager real worker e2e", () => {
  ...
  const run = async (task = "do it", timeoutMs = 2_000): Promise<AgentRunResult> => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-e2e-"));
    roots.push(root);
  ...
  describe("agent session usage export", () => {
    ...
    afterEach(() => {
      if (savedExportEnv === undefined) delete process.env.KIRO_FABRIC_AGENT_DIR;
      else process.env.KIRO_FABRIC_AGENT_DIR = savedExportEnv;
    ...
    const runExported = (sessionExport: boolean): Promise<AgentRunResult> => {
      savedExportEnv = savedExportEnv ?? process.env.KIRO_FABRIC_AGENT_DIR;
      const exportRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-export-e2e-"));
      exportRoots.push(exportRoot);
      process.env.KIRO_FABRIC_AGENT_DIR = exportRoot;
