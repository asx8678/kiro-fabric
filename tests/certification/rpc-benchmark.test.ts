...
describe("real benchmark safety gate", () => {
  it("is disabled by default and identifies every required opt-in", () => {
    ...
    expect(gate.reasons).toContain("KIRO_FABRIC_REAL_RESUME must equal 1");
  ...
  it("enables only with opt-in, model/provider, credential, repeats, budget, and pi-vcc", () => {
    const gate = benchmarkGate({
      ...
      KIRO_FABRIC_BENCH_MODEL: "model-id",
      KIRO_FABRIC_BENCH_PROVIDER: "provider-id",
      KIRO_FABRIC_BENCH_KEY_ENV: "TEST_MODEL_KEY",
      TEST_MODEL_KEY: "not-reported",
      KIRO_FABRIC_BENCH_REPEATS: "2",
      KIRO_FABRIC_BENCH_MAX_USD: "1.25",
  ...
  it("default benchmark command exits successfully with a skip report", () => {
    ...
    for (const key of Object.keys(env)) {
      if (key.startsWith("KIRO_FABRIC_BENCH") || key === "KIRO_FABRIC_REAL_RESUME" || key === "PI_VCC_EXTENSION") delete env[key];
    }
