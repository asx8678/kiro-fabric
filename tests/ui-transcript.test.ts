...
describe("agent transcript projection", () => {
  ...
  it("navigates an unbounded log through bounded lazy pages", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-transcript-"));
  ...
  it("appends explicit deltas while preserving legacy cumulative message_update snapshots", () => {
    const deltas = projectAgentTranscript([
      {
        type: "message_update",
        delta: true,
    ...
    expect(deltas.entries).toEqual([
      expect.objectContaining({ text: "First second", status: "running" }),
    ]);

    const legacySnapshots = projectAgentTranscript([
      { type: "message_update", message: { role: "assistant", content: "First" } },
      { type: "message_update", message: { role: "assistant", content: "First second" } },
    ]);
    expect(legacySnapshots.entries).toEqual([
      expect.objectContaining({ text: "First second", status: "running" }),
    ]);
  });

  it("keeps all streamed deltas in live and failed-run replay transcripts", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-transcript-delta-"));
    temporaryDirectories.push(directory);
    const logFile = path.join(directory, "events.jsonl");
    ...
    fs.writeFileSync(logFile, `${delta("streamed ")}\n`);
    const agent: FabricUiAgent = {
      id: "agent-delta",
      name: "delta",
      status: "running",
    ...
    const liveReader = new AgentTranscriptReader();

    expect(liveReader.read(agent).entries[0]).toMatchObject({
      text: "streamed ",
      status: "running",
    });
    fs.appendFileSync(logFile, `${delta("answer")}\n`);
    expect(liveReader.read(agent).entries[0]).toMatchObject({
      text: "streamed answer",
      status: "running",
    });
    const manyChunks = Array.from({ length: 60 }, (_, index) => `chunk-${index};`);
    fs.appendFileSync(logFile, `${manyChunks.map(delta).join("\n")}\n`);
    const expectedText = `streamed answer${manyChunks.join("")}`;
    expect(liveReader.read(agent).entries[0]).toMatchObject({
      text: expectedText,
      status: "running",
    });
    // A process failure can settle without message_end. The terminal run event
    // must retain and settle the reconstructed delta text.
    fs.appendFileSync(logFile, `${JSON.stringify({ type: "agent_end", status: "failed" })}\n`);
    expect(liveReader.read(agent).entries).toEqual([
      expect.objectContaining({ text: expectedText, status: "failed" }),
    ]);

    const replayed = new AgentTranscriptReader().read({ ...agent, status: "failed" });
