...
const fabricEnvKeys = [
  "KIRO_FABRIC_DEPTH",
  "KIRO_FABRIC_BUDGET",
  "KIRO_FABRIC_BUDGET_FILE",
  "KIRO_FABRIC_BUDGET_ID",
] as const;
...
describe("AgentManager", () => {
  it("notifies and releases UI subscribers", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-manager-"));
    roots.push(root);
  ...
  it("enforces a cross-process cost budget across spawned agents", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-budget-"));
    roots.push(root);
  ...
  it("inherits a budget ledger from the environment for recursive children", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-budget-"));
    roots.push(root);
    process.env.KIRO_FABRIC_BUDGET = "0.05";
    process.env.KIRO_FABRIC_BUDGET_FILE = path.join(root, "tree-cost.jsonl");
    process.env.KIRO_FABRIC_BUDGET_ID = "inherited-tree";
    fs.writeFileSync(process.env.KIRO_FABRIC_BUDGET_FILE, "", { mode: 0o600 });
    try {
      const manager = new AgentManager(process.cwd(), DEFAULT_FABRIC_CONFIG.agents, {
        workerPath: path.resolve("tests/fixtures/fake-worker-budget.mjs"),
        runRoot: root,
      ...
      const ledger = fs.readFileSync(process.env.KIRO_FABRIC_BUDGET_FILE, "utf8");
      expect(ledger).toContain("\"cost\":0.02");
    } finally {
      delete process.env.KIRO_FABRIC_BUDGET;
      delete process.env.KIRO_FABRIC_BUDGET_FILE;
      delete process.env.KIRO_FABRIC_BUDGET_ID;
    }
  });
  ...
  it("attributes token usage per tokens.usage events and closes the settle gap", async () => {
    ...
    process.env.KIRO_FABRIC_BUDGET = "0.05";
    process.env.KIRO_FABRIC_BUDGET_FILE = path.join(root, "tree-cost.jsonl");
    process.env.KIRO_FABRIC_BUDGET_ID = "attributed-tree";
    fs.writeFileSync(process.env.KIRO_FABRIC_BUDGET_FILE, "", { mode: 0o600 });
    try {
      ...
      const detail = readBudgetLedgerDetailed(process.env.KIRO_FABRIC_BUDGET_FILE);
  ...
  it("terminates a child that exceeds the per-child token limit", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-tokens-"));
    roots.push(root);
    const fakePi = path.resolve("tests/fixtures/fake-pi-rpc.mjs");
    fs.chmodSync(fakePi, 0o755);
...
describe("AgentManager multimodal prompts", () => {
  it("forwards image blocks to the Pi worker RPC prompt", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-images-"));
    roots.push(root);
...
describe("AgentManager Claude runner", () => {
  ...
  it("uses the independent configured Claude runner and model defaults", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-claude-"));
    roots.push(root);
    const config = {
...
describe("AgentManager steering", () => {
  ...
  it("steer appends a queued steer command for a running agent", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-steer-"));
    roots.push(root);
