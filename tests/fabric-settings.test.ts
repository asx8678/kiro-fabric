...
describe("FabricSettingsComponent", () => {
  // KIRO_FABRIC_FULL_CODE_MODE is a process-level override (src/config.ts), so an
  // outer Fabric session would silently defeat every settings-persistence
  // assertion here. Isolate it for this suite.
  const inheritedFullCodeMode = process.env.KIRO_FABRIC_FULL_CODE_MODE;
  beforeEach(() => {
    delete process.env.KIRO_FABRIC_FULL_CODE_MODE;
  });
  afterEach(() => {
    if (inheritedFullCodeMode === undefined) delete process.env.KIRO_FABRIC_FULL_CODE_MODE;
    else process.env.KIRO_FABRIC_FULL_CODE_MODE = inheritedFullCodeMode;
  });
  ...
  it("persists tool-display changes through the real settings dialog flow", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-settings-display-"));
  ...
  it("persists trusted-project changes globally after Ctrl+G", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-settings-global-"));
    const cwd = path.join(root, "project");
    const agentDir = path.join(root, "agent");
    const inheritedAgentDir = process.env.PI_CODING_AGENT_DIR;
    fs.mkdirSync(cwd, { recursive: true });
  ...
  it("keeps global edits visible when a project override remains effective", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-settings-shadowed-global-"));
    const cwd = path.join(root, "project");
    const agentDir = path.join(root, "agent");
    const inheritedAgentDir = process.env.PI_CODING_AGENT_DIR;
    const inheritedFullCodeMode = process.env.KIRO_FABRIC_FULL_CODE_MODE;
  ...
  it("persists a picked Prewalk thinking level through the real settings dialog flow", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-settings-thinking-"));
  ...
  it("persists a picked Prewalk model through the real settings dialog flow", async () => {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-settings-model-"));
    // Isolate the agent dir: the settings dialog layers the real global
    // fabric.json under the project layer, so the developer's global prewalk
    // config would otherwise leak into the rendered labels.
    const inheritedAgentDir = process.env.PI_CODING_AGENT_DIR;
