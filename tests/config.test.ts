...
afterEach(() => {
  ...
  if (originalCompactionEngineEnv === undefined) {
    ...
    process.env.KIRO_FABRIC_COMPACTION_ENGINE = originalCompactionEngineEnv;
  }
});
...
describe("Fabric configuration", () => {
  ...
  it("bounds fabric source bytes to a non-configurable safety ceiling", () => {
    expect(DEFAULT_FABRIC_CONFIG.executor.maxSourceBytes).toBe(DEFAULT_EXECUTOR_SOURCE_BYTES);
    expect(normalizeFabricConfig({ executor: { maxSourceBytes: 1 } }).executor.maxSourceBytes)
      .toBe(MIN_EXECUTOR_SOURCE_BYTES);
    expect(normalizeFabricConfig({
      executor: { maxSourceBytes: Number.MAX_SAFE_INTEGER },
    }).executor.maxSourceBytes).toBe(MAX_EXECUTOR_SOURCE_BYTES);
  });
  ...
  it("normalizes the default runner and independent Claude settings", () => {
    ...
    const kiro = normalizeFabricConfig({ agents: { runner: "kiro" } });
    expect(kiro.agents.runner).toBe("kiro");
    expect(() =>
      normalizeFabricConfig({
        agents: { runner: "other", claude: { binary: " ", model: " " } },
      }),
    ).toThrow(/Unsupported Fabric agent runner: other/);
  });
  ...
  it("normalizes the Veda runner settings", () => {
    ...
    expect(() => normalizeFabricConfig({ agents: { runner: "other" } })).toThrow(
      /Unsupported Fabric agent runner: other/,
    );
    expect(() => normalizeFabricConfig({ agents: { runner: null } })).toThrow(
      /Unsupported Fabric agent runner/,
    );
    expect(() => normalizeFabricConfig({ agents: { runner: 42 } })).toThrow(
      /Unsupported Fabric agent runner/,
    );
  });
  ...
  it("updates the compaction engine environment across config re-initialization", () => {
    ...
    expect(process.env.KIRO_FABRIC_COMPACTION_ENGINE).toBe("fabric");

    fs.writeFileSync(projectConfig, JSON.stringify({ compaction: { engine: "pi" } }));
    loadFabricConfig({ cwd, agentDir, projectTrusted: true });
    expect(process.env.KIRO_FABRIC_COMPACTION_ENGINE).toBeUndefined();
  });
  ...
  it("saves and reloads a Kiro runner as config version 4", () => {
    const root = temporaryDirectory();
    const cwd = path.join(root, "project");
    const agentDir = path.join(root, "agent");
    fs.mkdirSync(path.join(cwd, ".pi"), { recursive: true });
    ...
    const saved = JSON.parse(fs.readFileSync(path.join(cwd, ".pi", "fabric.json"), "utf8"));
    expect(saved).toMatchObject({ configVersion: 4, agents: { runner: "kiro" } });
    expect(loadFabricConfig({ cwd, agentDir, projectTrusted: true }).agents.runner).toBe("kiro");
  });

  it("rejects an unknown runner supplied to save without modifying the target", () => {
    const root = temporaryDirectory();
    const cwd = path.join(root, "project");
    const agentDir = path.join(root, "agent");
    fs.mkdirSync(path.join(cwd, ".pi"), { recursive: true });
    fs.mkdirSync(agentDir, { recursive: true });
    const target = path.join(cwd, ".pi", "fabric.json");
    const existing = JSON.stringify({ configVersion: 4, agents: { runner: "pi" } }, null, 2);
    ...
    expect(() =>
      saveFabricConfig(
        { cwd, agentDir, projectTrusted: true },
        { agents: { runner: "future-runner" } },
      ),
    ).toThrow(/Unsupported Fabric agent runner: future-runner/);
