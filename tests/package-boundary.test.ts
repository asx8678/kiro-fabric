...
describe("Agent product boundary", () => {
  ...
  it("keeps the checkout-local Agent launchable and CI on Agent tests", () => {
    ...
    expect(profile.mcpServers.fabric).toEqual({ command: "node", args: [".kiro/agents/run-fabric-mcp.mjs"] });
    expect(profile.mcpServers.fabric.cwd).toBeUndefined();
    expect(profile.resources).toEqual(["skill://../../skills/fabric-exec/SKILL.md", "file://../steering/fabric.md"]);
    const launcher = path.join(root, ...profile.mcpServers.fabric.args[0].split("/"));
    expect(fs.existsSync(launcher)).toBe(true);
    const fromRepo = spawnSync(process.execPath, profile.mcpServers.fabric.args, {
      cwd: root,
      encoding: "utf8",
      input: "",
      timeout: 30_000,
    });
    expect(fromRepo.status, fromRepo.stderr).toBe(0);
    const fromParent = spawnSync(process.execPath, [launcher], {
      cwd: path.dirname(root),
      encoding: "utf8",
      input: "",
      timeout: 30_000,
    });
    expect(fromParent.status, fromParent.stderr).toBe(0);
