...
const createStore = (options?: MeshStoreOptions): MeshStore => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-mesh-"));
  roots.push(root);
...
describe("MeshStore", () => {
  ...
  it("supports complete internal prefix scans independently of public read limits", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-mesh-scan-"));
    roots.push(root);
  ...
  it("compacts oversized event logs and resets stale tail cursors", async () => {
    const meshRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-mesh-bounded-"));
    roots.push(meshRoot);
...
describe("MeshStore lock recovery", () => {
  ...
  it("publishes a batch with contiguous sequences in input order", async () => {
    const store = createStore();
    const events = await store.publishBatch([
      { topic: "team.auth", from: identity, text: "one" },
      { topic: "team.auth", from: identity, text: "two", to: "b" },
      { topic: "team.auth", from: identity, text: "three", data: { n: 3 } },
    ]);

    expect(events.map((e) => e.sequence)).toEqual([1, 2, 3]);
    expect(events.map((e) => e.text)).toEqual(["one", "two", "three"]);
    // The next scalar publish continues the sequence.
    const next = await store.publish({ topic: "team.auth", from: identity, text: "four" });
    expect(next.sequence).toBe(4);
    // All events are readable through the cursor.
    const read = store.read({ after: 0 });
    expect(read.map((e) => e.sequence)).toEqual([1, 2, 3, 4]);
  });

  it("preflights validation so a bad batch mutates neither log nor counter", async () => {
    const store = createStore();
    const before = store.latestSequence();
    await expect(
      store.publishBatch([
        { topic: "team.auth", from: identity, text: "ok" },
        { topic: "   ", from: identity, text: "bad" },
      ]),
    ).rejects.toThrow();
    expect(store.latestSequence()).toBe(before);
    expect(store.read({ after: 0 })).toHaveLength(0);
  });

  it("treats an empty batch as a no-op", async () => {
    const store = createStore();
    await expect(store.publishBatch([])).resolves.toEqual([]);
