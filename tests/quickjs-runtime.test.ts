...
describe("QuickJsRuntime", () => {
  it("rejects oversized UTF-8 source before transpilation", async () => {
  ...
  it("rejects oversized caller-supplied transpiled code before QuickJS startup", async () => {
    const hostCall = vi.fn(async () => undefined);
    const result = await new QuickJsRuntime().execute(
      "return 1;",
      hostCall,
...
`,
  ...
  it("forwards cwd through the workflow leaf helper", async () => {
    let request: Record<string, unknown> | undefined;
    const result = await new QuickJsRuntime().execute(
      `return workflow.agent("workflow task", { cwd: "workflow-target" });`,
      async (ref, args) => {
        if (ref === "agents.run") {
          request = args;
    ...
    expect(result.error).toBeUndefined();
    expect(request).toMatchObject({ task: "workflow task", cwd: "workflow-target" });
  });

  it("rejects unaccounted Kiro workflow runners before launching a child", async () => {
    const hostCall = vi.fn(async () => ({
      status: "completed",
      usage: { availability: "unavailable" },
    }));
    for (const code of [
      'return workflow.agent("must not launch");',
      'return workflow.agent("must not launch", { runner: "kiro" });',
    ]) {
      const result = await new QuickJsRuntime().execute(code, hostCall, {
        ...options,
        tokenBudget: 100,
        defaultAgentRunner: "kiro",
        unaccountedAgentRunners: ["kiro"],
      });
      expect(result.error).toMatch(/cannot launch runner kiro.*usage is unavailable/i);
    }
    expect(hostCall).not.toHaveBeenCalled();
  });
...
`,
  ...
  it("routes every actor-management guest proxy to the agents provider", async () => {
    const calls: Array<{ ref: string; args: Record<string, unknown> }> = [];
    const result = await new QuickJsRuntime().execute(
      `
...
await agents.import({ name: "template", as: "local" });
await agents.export({ id: "a1", overwrite: true });
return { done: true };
`,
      async (ref, args) => {
        calls.push({ ref, args });
        return { id: String(args.id ?? "template"), status: "idle", name: "x" };
      },
      options,
    );
    expect(result.error).toBeUndefined();
    ...
    expect(calls).toEqual([
      { ref: "agents.setTools", args: { id: "a1", tools: ["read"] } },
      { ref: "agents.setEvents", args: { id: "a1", events: ["turn_end"] } },
      ...
      { ref: "agents.clearMessages", args: { id: "a1" } },
      { ref: "agents.import", args: { name: "template", as: "local" } },
      { ref: "agents.export", args: { id: "a1", overwrite: true } },
    ]);
  });
});
...
describe("QuickJsRuntime guest stack remapping", () => {
  ...
  it("keeps emitted frames when no source map accompanies pre-transpiled code", async () => {
    ...
    expect(result.terminationReason).toBe("runtime_error");
    expect(result.error).toContain("kiro-fabric-guest.js");
  });
});
