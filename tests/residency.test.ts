...
const initRepository = (directory: string): void => {
  ...
  git(directory, "config", "user.email", "kiro-fabric-tests@example.invalid");
...
const rootHarness = async (name: string): Promise<RootHarness> => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `kiro-fabric-${name}-`));
  roots.push(root);
...
describe.skipIf(!hasResidentHost)("durable participant residency", () => {
  ...
  it("rejects tampered durable worktree metadata before destructive cleanup", async () => {
    ...
    const branch = `kiro-fabric/tampered-${id.slice(0, 8)}`;
    const worktree = path.join(os.tmpdir(), "kiro-fabric-worktrees", id);
