...
const initRepository = (prefix: string, relativeDirectory?: string): string => {
  ...
  git(repository, "config", "user.email", "kiro-fabric-tests@example.invalid");
...
describe("one-shot agent cwd", () => {
  it("canonicalizes absolute, relative, and symlink paths through launch and reporting", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-cwd-"));
    roots.push(root);
  ...
  it("rejects cwd on recursive requests before creating a run directory", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-cwd-recursive-"));
    roots.push(root);
  ...
  it.skipIf(process.platform === "win32" || process.getuid?.() === 0)(
    ...
    async () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-cwd-inaccessible-"));
      roots.push(root);
  ...
  it("reuses the selected repository and subdirectory for worktree launches", async () => {
    const parent = initRepository("kiro-fabric-parent-repo-");
    const target = initRepository("kiro-fabric-target-repo-", path.join("packages", "app"));
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-cwd-worktree-"));
    roots.push(root);
    const manager = createManager(parent, path.join(root, "runs"), createWorker(root), {
      projectRoot: parent,
      meshRoot: path.join(root, "mesh"),
  ...
  it("keeps the generated worktree root when cwd is omitted from a subdirectory manager", async () => {
    const repository = initRepository("kiro-fabric-default-worktree-repo-", path.join("packages", "app"));
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-default-worktree-"));
    roots.push(root);
  ...
  it("does not create a worktree from the parent repository for a non-Git cwd", async () => {
    const parent = initRepository("kiro-fabric-parent-repo-");
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-cwd-nongit-"));
    roots.push(root);
  ...
  it("retains the selected cwd across startup retries", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-cwd-retry-"));
    roots.push(root);
    const target = path.join(root, "target");
    fs.mkdirSync(target);
    const manager = createManager(
