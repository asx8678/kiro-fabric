...
const project = (config: Record<string, unknown> = {}): string => {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "kiro-fabric-bootstrap-"));
...
const runtimeHarness = () => {
  ...
  class FakeRuntime {
    initialized = false;
    widgetDismissedAt = 0;
    mcpDescriptors: FabricActionDescriptor[] = [];
    readonly mcpHooks: { onSliceChanged?: (descriptors: FabricActionDescriptor[]) => void } | undefined;
    ...
    constructor(...args: unknown[]) {
      this.mcpHooks = args[3] as typeof this.mcpHooks;
      instances.push(this);
    }
    mcpSlice = (): FabricActionDescriptor[] => this.mcpDescriptors;
    emitMcpSlice(descriptors: FabricActionDescriptor[]): void {
      this.mcpDescriptors = descriptors;
...
describe("FabricState lazy bootstrap", () => {
  ...
  it("serves a same-layer MCP advisory cache before loading the runtime", async () => {
    ...
    // Bootstrap descriptors remain visible until hydration reports its first
    // authoritative live slice, but an intentionally empty live slice must
    // suppress rather than resurrect the bootstrap cache.
    expect(state.mcpSlice()).toEqual([
      expect.objectContaining({ name: "cached.echo" }),
    ]);
    harness.instances[0]!.emitMcpSlice([]);
    expect(state.mcpSlice()).toEqual([]);
  ...
  it("eagerly detects always-rearm and only nonempty valid actor registries", async () => {
    ...
    vi.stubEnv("KIRO_FABRIC_ACTOR_ID", "");
    vi.stubEnv("KIRO_FABRIC_PARENT_RUN", "");
    vi.stubEnv("KIRO_FABRIC_MESH_ROOT", path.join(actorCwd, ".pi", "fabric", "mesh"));
  ...
  it("eagerly activates child runtimes with inherited capability commitments", async () => {
    ...
    const context = contextAt(cwd);
    await state.bootstrap(context);
    vi.stubEnv("KIRO_FABRIC_CAPABILITY_REQUIREMENTS", JSON.stringify(["memory.recall"]));
    vi.stubEnv("KIRO_FABRIC_CAPABILITY_DIGEST", "semantic-digest");
    expect(state.shouldEagerlyActivate(context)).toBe(true);
    vi.unstubAllEnvs();
  });
  ...
  it("does not probe persisted actors from nested actor or agent identities", async () => {
    ...
    const context = contextAt(cwd);
    await state.bootstrap(context);
    vi.stubEnv("KIRO_FABRIC_MESH_ROOT", path.join(cwd, ".pi", "fabric", "mesh"));

    vi.stubEnv("KIRO_FABRIC_ACTOR_ID", "actor-child");
    expect(state.shouldEagerlyActivate(context)).toBe(false);
    vi.stubEnv("KIRO_FABRIC_ACTOR_ID", "");
    vi.stubEnv("KIRO_FABRIC_PARENT_RUN", "agent-child");
    expect(state.shouldEagerlyActivate(context)).toBe(false);
    vi.stubEnv("KIRO_FABRIC_PARENT_RUN", "");
    expect(state.shouldEagerlyActivate(context)).toBe(true);
    vi.unstubAllEnvs();
