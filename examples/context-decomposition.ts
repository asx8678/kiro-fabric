...
const reports = await fabric.ai.parallel({
  tasks: await Promise.all(
    partitions.map(async (partition) => ({
      ...
      instruction:
        "Audit only this finite partition. Do not repartition, delegate, recurse, or call Fabric Lite. Cite path/line/quote evidence.",
      context: await fabric.fs.readMany({ paths: partition, maxCharsPerFile: 4000 }),
      outputSchema: {
        type: "object",
...
return {
  status: reports.every((report) => report.error)
    ? "failed"
