...
The release package contains no session wrapper, custom-agent selector, extension-host adapter, or generic MCP discovery surface. Ordinary build, test, hermetic certification, packaging, and release-report commands are guarded against `$KIRO_HOME` mutation. The explicit real-client release gate uses operator-owned Kiro state but confines requested native file and shell probes to a private empty temporary working directory.
